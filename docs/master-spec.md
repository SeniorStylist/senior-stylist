...
| Native app shell | **Capacitor** (`@capacitor/core/cli/ios/android`) — `capacitor.config.ts` loads the live Vercel site in a managed WebView (`appId: com.seniorstylist.app`, `CAP_SERVER_URL ?? https://portal.seniorstylist.com`). Generated `android/`+`ios/` committed; excluded from tsconfig+eslint. `isNativeApp()`/`nativePlatform()` in `src/lib/detect-device.ts`. Build/submit: `docs/native-app.md`. Backend unchanged. |
...
| `/billing/monthly` | master / `canAccessBilling` → redirect `/dashboard` | Monthly facility statement (2026-06-12) — facility identity header (name + FID chip + IP/RFMS/Hybrid chip, always visible), month cards (invoiced/services/collected/owed + progress bar + services-vs-invoiced Δ chip; negative open balance renders as a sky "Credit" tile), expandable per-month detail: By resident / By day pill toggle (resident table has sortable columns — name/services/invoiced/paid/owed; day view groups per-booking service rows with unpaid chips), invoices, payments (camera icon → check-image lightbox). Facility combobox for master+bookkeeper; "Monthly view →" link on /billing facility card |
...
- `POST /api/log/ocr` — upload + OCR log sheet images (**P37: also allows `stylist`** — own-sheet scanning; the import route enforces own-stylist/own-facility)
- `POST /api/log/ocr/import` — commit OCR-reviewed entries to bookings (**P37: also allows `stylist`** with forced own stylistId)

**`PUT /api/bookings/[id]` — bookkeeper field restriction**: bookkeeper is now allowed but server-side strips disallowed fields. `BOOKKEEPER_ALLOWED` set: `residentId, serviceId, serviceIds, addonServiceIds, addonChecked, priceCents, paymentStatus, paymentMethod, notes, tipCents, selectedQuantity, selectedOption, roomNumber, residentName, startTime, stylistId`. Fields `status`, `cancellationReason` are silently dropped — bookkeeper cannot change appointment status or cancel via PUT. `roomNumber` and **`residentName` (R9 2026-08-11)** are RESIDENT-field passengers: applied to the booking's effective resident row (skipped client-side when the booking is re-pointed), never bookings columns.
...
- **`payment_method`**: text, nullable — free-text payment-type label paired with `payment_status`. Decision (2026-06-26): Cash/Check/Card/ACH = collected (`payment_status='paid'`); **RFMS/COF/RA and any custom label = OPEN INVOICE** (`payment_status='unpaid'`, still counts toward outstanding balance) — the label is for display/export only. **2026-07-13: `'None'` is a first-class option** (bookkeeper request; Josh: open invoice) — `parsePaymentCombo` maps `none` / `no payment type` / `no payment` → `{paymentStatus:'unpaid', paymentMethod:'None'}` and a one-time prod UPDATE normalized 20 hand-typed variants. Parsed/seeded via `src/lib/payments.ts` (`parsePaymentCombo`/`comboLabel`). The daily-log Excel export's "Payment Type" column = `paymentTypeLabel(payment_status, payment_method)` (method wins; falls back to Invoice/Waived/Other).
- **`mail_subject`**: text, nullable (`drizzle/0015_booking_mail_subject.sql`) — per-log-sheet "Mail Subject" entered at OCR-scan time (`bookings.mail_subject` written by `POST /api/log/ocr/import` from the per-sheet field). Drives column B of the daily-log Excel export; the export-modal subject is the fallback when null.
...
| `/residents/[id]` | Resident detail + collapsible **Account ledger** card (`<ResidentLedger>`, billing roles, lazy-loaded; invoices/payments/credits + apply-credit-to-invoice) |
| `/signage` | **Admin + facility_staff** (2026-06-26) — editable signage templates (General/Holiday) → live iframe preview → "Print / Save PDF" (new-window `window.print()`, no PDF dep). Management nav group (sidebar only). |
| `/franchise` | **Franchise admin (`isFranchiseAdmin`) or master** (2026-06-26) — scoped mini-master dashboard: the franchise's facilities grid (per-facility MTD appointments/collected/outstanding) + franchise totals; each card switches the active facility (`/api/facilities/select`) and opens its billing/dashboard. Sidebar "Franchise" link gated on `isFranchiseAdmin`. |
...
| `/settings` | Apple-style two-pane settings (Phase 11J.3; shell redesigned 2026-06-15). Seven categories: General, Team & Roles, Family Portal, Billing & Payments, Integrations, Notifications, Advanced — grouped in the rail under Facility / People & Access / Billing / System with a lucide icon each, burgundy-tinted active state, a rail search box (admin, >4 categories), and a consistent per-section header (icon chip + title + description) rendered by the shell. URL convention: `?section=<id>` (legacy `?tab=<id>` resolves via back-compat map). Role-gated: admin sees all; facility_staff sees only General (read-only); bookkeeper sees only Notifications (read-only); stylist/viewer redirected by `page.tsx` guard. Sections under `src/app/(protected)/settings/sections/`. Rev-share toggle was moved here from `/billing` (Phase 11J.3). |
...
- **Public** (no Supabase session required): `/login`, `/auth`, `/unauthorized`, `/invite/accept`, `/portal/*`, `/api/portal/*`, `/invoice/*`, `/family/*`, `/api/cron/*`, `/privacy`, `/terms`, and **`/api/access-requests`** (2026-06-23 — the access-request POST was being intercepted + redirected to an HTML page, breaking the JSON form). See the `skipSupabase` short-circuit list (Performance section in CLAUDE.md) for the full set.
...
| `logEmail` | 10 / hour | user id | `POST /api/log/email` (email day log) |
...
| `POST /api/log/ocr/import` | canScanLogs / master / **stylist (own-sheet, P37)** | Accepts optional `facilityId` (bookkeeper/master cross-facility, IDOR-validated); stores confirmed sheets on `import_batches.review_payload`. Stylist callers: facility pinned, identity via `getEffectiveStylistId` (null → human 403), every sheet's `stylistId` FORCED to own id + `stylistName` cleared (no stylist creation) |
| `POST /api/log/ocr/batch/[id]/undo` | canScanLogs / master / **stylist for own uploads (P37)** | Rolls back an OCR scan batch (soft-delete bookings) and returns its saved review sheets for "Undo & edit"; stylists only when `uploadedBy === user.id` |
...
| `GET /api/exports/daily-logs` | Authenticated; rate-limited `exportExcel` (Phase 12Z) | Query: `facilityIds` (the literal **`all`** — expanded server-side to every facility the caller can access, added 2026-07-05 for the 109-facility "Select all" export — or CSV of 1–200 UUIDs; was capped at 50 which made all-facility exports 400), `startDate`, `endDate` (`YYYY-MM-DD`, ≤366d). Client modals call it via `downloadExportFile()` (`src/lib/exports/download-export.ts`) — fetch + blob download with toast on error JSON; never `window.open`. Emits styled `.xlsx` (exceljs) matching the bookkeeper accounting format (14 columns: `No., Mail Subject, Mail date, Mail Time, Service Date, Facility Name, Stylist Name, Client Name, Room#, Services Performed, Amount, Notes, Tips, Payment Type`). Yellow header fill (`FFFFFF00`), Calibri, bold all except B/C/D. Column widths C=11.33 D=9.66 E=12.89 F=26.66 G=20.55 H=22.44 N=14.22. Filters: `bookings.status='completed' AND active=true AND startTime` within `[startDate, endDate]` per-facility tz (uses `dayRangeInTimezone` to compute the widest UTC overlap then per-row tz re-filter). Scoping: master → any facility; admin/bookkeeper/facility_staff → restricted to `facilityUsers` memberships (403 on out-of-scope); stylist → forced to own facility + `bookings.stylistId = profiles.stylistId`. Payment Type mapping (col N) via `paymentTypeLabel(payment_status, payment_method)` (2026-06-26): the `payment_method` label wins when present (→ "Cash"/"Check"/"Card"/"ACH"/"RFMS"/"COF"/"RA"/custom); else `unpaid → "Invoice"`, `waived → "Waived"`, `paid → "Other"`, null → `"Doesn't Fill"` (no `qb_payments` join). Column B "Mail Subject" = `booking.mail_subject ?? <mailSubject query param> ?? "Senior Stylist Export"` — `mailSubject` is an optional query param (≤200 chars) set by the export modals; per-booking `mail_subject` (entered at OCR-scan time) wins. "Doesn't Fill" placeholder used for: empty room, empty notes, null/zero tips, unknown payment type, missing services. Stylist + facility codes pulled from `stylists.stylist_code` (ST###, NOT NULL) and `facilities.facility_code` (text, nullable — falls back to bare name when null). Filename: `{facilityCodeOrName}_{startDate}_to_{endDate}.xlsx` (single-facility) or `multi-facility_{startDate}_to_{endDate}.xlsx`. **Empty-facility warning (2026-07-27)**: response carries `X-Export-Empty-Facilities` (URI-encoded JSON array of `"F149 - Name"` labels, capped 10 + `+N more`) listing requested facilities that produced ZERO rows; `downloadExportFile` decodes it into `{ ok: true, warning }` and both export modals toast it with the persistent error variant — a facility silently missing from the file (rolled-back sheet / misread date) must never go unnoticed. |
| `POST /api/log/ocr` | Authenticated | Accept `images[]` + optional `servicesJson` (JSON `{ name, priceCents }[]`). Route calls `buildInstruction(knownServices)` to inject facility service list + abbreviation table into the Gemini prompt so it can expand shorthand (e.g. "S/BDry" → "Shampoo, Blow Dry") and use price as a matching signal. Prompt instructs Gemini to extract EXACT written price (never substitute catalog price) and write full legible names. `maxDuration = 120`, per-file timeout 90s. Returns `{ data: { sheets } }`. **2026-07-05:** the prompt also extracts **`facilityName`** (facility/community name printed on the sheet); the route fuzzy-matches it against active non-demo facilities (exact F-code match first, then `fuzzyBestMatch` ≥0.7) and each sheet gains **`detectedFacility: { id, name, facilityCode } \| null`** — drives the wrong-facility mismatch banner in the review step. Review step service fields are `<select>` dropdowns pre-populated via two-signal matching: name fuzzy score + exact price match (`s.priceCents === ocrPrice`); price match wins when nameScore < 0.85 and only one service at that price; price from sheet is the source of truth and is never overwritten by service selection. |
| `GET /api/log/ocr/rosters` | canScanLogs + master (2026-07-05) | Query `facilityId=<uuid>`. Returns `{ residents (id/name/roomNumber), stylists (id/name), services (id/name/priceCents/pricingType) }` for the TARGET facility (`active=true`, `isDemo=false`, name-ordered). Cross-facility: bookkeeper + master only (others 403 unless own facility); unknown/inactive facility → 404. Used by the scan modal so cross-facility scans send the right rosters to Gemini and review dropdowns show the target facility's records. |
| `GET /api/log/import-batches` | canScanLogs + master | Log Sheet History list (the "Sheets" button on /log → `LogSheetsModal`). `sourceType='ocr_scan'` batches, newest 200: master → all; bookkeeper → own uploads (cross-facility); admin/staff → own facility. Rows include facility name/code + uploader name + `deletedAt` ("Rolled back" badge) + **`activeBookingCount` (2026-07-27)** — ONE `GROUP BY import_batch_id WHERE bookings.active=true` over the listed batches; `LogSheetsModal` shows "N bookings on file" and renders amber treatment + explanation for rolled-back batches and for live batches with 0 active bookings (self-serve "why is my sheet missing from the export" diagnostic). **Move-picker data source (2026-07-12)**: `LogSheetsModal` takes a `facilities` prop fed from the /log page's `exportFacilities` (all active non-demo facilities for bookkeeper/master) — NEVER populate it from `GET /api/facilities`, which is membership-scoped and returns only the bookkeeper's single anchor row (the "couldn't move" bug). `log/page.tsx` also threads `isMaster` → `LogClient` → `isMasterAdmin` so master admin gets the Move button. |
| `PUT /api/log/import-batches/[batchId]` | canScanLogs + master; `move` is bookkeeper/master only | Body `{action:'move', targetFacilityId}` or `{action:'rename', name}`. **Move (rewritten 2026-07-05)** re-points the WHOLE batch to the target facility in one transaction: per distinct resident — fuzzy-match in target ≥0.8 → else MOVE the resident row when this batch is its only active usage → else CLONE into target (fresh portalToken); services are always find-or-create in target (≥0.8 name match, clones get `source:'ocr_import'`; service rows are never moved — other bookings may share them); every booking gets `facilityId` + remapped `residentId`/`serviceId`/`serviceIds[]`. The old version flipped only `bookings.facility_id` and left residents/services in the source facility (scoping corruption). Rename updates `facilities.name`. |
| `DELETE /api/log/import-batches/[batchId]` | canScanLogs + master (bookkeeper: own uploads; admin: own facility) | Roll back a scan batch — `bookings.active=false` + `import_batches.deleted_at=now()`; returns `bookingsDeactivated`. |
| `POST /api/log/ocr/import` | **Admin / bookkeeper / facility_staff** (`canScanLogs`) + master | Create missing **stylists** + residents + services + multi-service completed bookings from reviewed sheets in one `db.transaction()`; bookings spaced 30 min from 09:00 UTC. Each sheet sends `stylistId: string \| null` + `stylistName: string` — a chosen id is IDOR-validated against the facility (owned or active assignment, else 403); when no id, the name is fuzzy-matched (≥0.8) or a new stylist is created (`generateStylistCode` → `ST###`, scoped `facilityId`+`franchiseId`, with a `stylist_facility_assignments` row), mirroring the multi-log importer. Accepts `additionalServiceIds: (string \| null)[]` + `additionalServiceNames: string[]` per entry; resolves each via the 3-step fuzzy-match algorithm and stores all IDs in `service_ids`. **2026-06-26:** sheet accepts `mailSubject: string` (≤200, written to each booking's `mail_subject`); resident **room update is now unconditional** when `entry.roomNumber` is provided (was guarded on `isNull(residents.roomNumber)`) — residents change rooms, the sheet wins (blanks skipped). Per-entry `paymentStatus`/`paymentMethod` come from the review modal's payment `<select>` via `parsePaymentCombo` (`src/lib/payments.ts`). Returns `created: { residents, services, stylists, bookings }` |
...
| `POST /api/residents/bulk` | **Admin** (`isAdminOrAbove`) | Bulk insert residents (conflict skip on name+facility) |
...
| `POST /api/stylists/[id]/invite` | **Admin** | Send a **branded** invite to the stylist's email (2026-06-23: replaced `admin.inviteUserByEmail` magic link, which sent an unbranded email and created no `facility_users` row). Now uses the same `invites` table + Resend `/invite/accept?token=` flow as Settings → Team: re-sends/refreshes an existing pending invite, 409 if the stylist already has active access. Guards: email must exist (400), franchise scope (403), 24h rate limit (429). Updates `stylists.last_invite_sent_at`. On redeem (`GET /api/invite/redeem`), `profiles.stylistId` auto-links by **email match first** (`ilike(stylists.email, …)`), then name. |
...
| `GET/POST /api/services` | Authenticated | List/create services. POST accepts `pricingType`, `addonAmountCents`, `pricingTiers`, `pricingOptions` with `.refine()` validation. **POST/PUT/DELETE + bulk/bulk-update/reorder/parse-pdf now allow `facility_staff`** (2026-06-26) via `!isAdminOrAbove(role) && !isFacilityStaff(role)` — front desk sets up the service menu. |
| `GET/PUT/DELETE /api/services/[id]` | Authenticated | Single service. PUT accepts same pricing fields as POST |
| `POST /api/services/bulk` | **Admin** (`isAdminOrAbove`) | Bulk insert services (conflict skip on name+facility) |
| `POST /api/services/bulk-update` | Authenticated | Bulk update `color` or `active` for a set of service IDs scoped to facility |
| `POST /api/services/parse-pdf` | Admin | Extract services from a price sheet using **Gemini 2.5 Flash** (`temperature: 0`, JSON mode). Accepts EITHER `file` (a PDF, sent as native `application/pdf` inlineData) OR `gridText` (a spreadsheet's full cell grid as tab-separated text — `xlsx`/`xls`/`csv`, all sheets). No regex/column-mapping — Gemini reads the layout and ignores section headers, prose, dates, page markers, and blank rows. `gridText` capped at 400k chars. **Response shape** `{ data: { facilityName: string \| null, rows: [...] } }` — Gemini also extracts the facility/salon name from the top of the document as STEP 1 (explicitly excludes "Senior Stylist" as the provider name); old bare-array response still accepted by clients for backward compat. `rows` contains `name, priceCents, durationMinutes, category, color, pricingType, addonAmountCents, pricingTiers, pricingOptions`. **`parsePriceSheetFile` returns `ParseResult = { rows, detectedFacilityName }`** — callers in the bulk price sheets uploader use `detectedFacilityName` as a content-based fallback for facility matching (threshold 0.65) when filename detection fails. **Spreadsheets route through the SAME parser as PDFs** (client converts to grid text in `import-client.tsx` via `spreadsheetToGridText`); the old naive `parseSpreadsheetNaive` column-mapping remains only as a fallback when Gemini is unavailable. Switched off naive column-mapping because real facility price sheets are free-form (irregular columns, section headers, policy prose) and mapped to garbage rows |
...
| `PUT /api/facility/users/[userId]/stylist` | **Admin** | Assign/change/remove a team member's stylist directory link. Body `{ stylistId: uuid \| null }` (null = unlink) → sets `profiles.stylistId` for the target member. Guards: target must be a `facility_users` member at the admin's facility (404), stylist must belong to the facility + be active (404), 409 takeover guard if another profile already holds that stylist. Returns `{ data: { stylistId, stylistName } }`. UI: interactive picker in Settings → Team & Roles (`team-section.tsx`). The link drives the stylist's daily-log/check-in/export `stylistFilter` scope. |
| `GET/POST /api/facility/coupons` | **Admin** | List facility coupons (with per-coupon `redemptionCount`) / create a coupon (`portal_coupons`). POST body: `code?` (auto-gen `SAVE-XXXXXX` if omitted; global-unique, retries on collision), `type` (welcome\|birthday\|referral\|loyalty\|manual), `discountType` (percent\|fixed), `discountValue` (1–100 for percent, cents for fixed), `description?`, `maxRedemptions?`, `maxPerAccount` (default 1), `expiresAt?`. (2026-06-26) |
| `PATCH /api/facility/coupons/[couponId]` | **Admin** | Edit/deactivate a coupon (scoped to the admin's facility). Partial update of discountType/value/description/maxRedemptions/maxPerAccount/expiresAt/active. |
| `POST /api/facility/coupons/[couponId]/issue` | **Admin** | Issue a coupon to a family. Body `{ residentId, portalAccountId? }` → creates `portal_coupon_redemptions` for every portal account linked to the resident (or one specific account), respecting `maxRedemptions` (global) + `maxPerAccount` + expiry + active (409 on violation). 409 if the resident has no linked portal account. |
| `GET /api/facility/coupons/recipients` | **Admin** | Residents at the facility that have ≥1 linked portal account — the valid coupon recipients (one row per resident×account: `residentId, residentName, roomNumber, portalAccountId, email`). |
| `GET /api/residents/[id]/ledger` | **canAccessBilling** (master/bookkeeper cross-facility, admin own) | Per-resident account ledger: `summary` (currentBalanceCents from `residents.qb_outstanding_balance_cents`, availableCreditCents, totalInvoicedCents, totalPaidCents), `entries` (invoices+payments interleaved chronologically with a running balance, newest-first), `credits` (remaining unapplied = `open_balance_cents - applied_cents`), `openInvoices` (for the apply picker). Graceful fallback when `qb_unapplied_credits` is pre-migration. (2026-06-26) |
| `POST /api/residents/[id]/credits/[creditId]/apply` | **canAccessBilling** | Manually attribute an account credit (prepay/gift/coupon) to chosen open invoices. Body `{ invoiceIds: uuid[1..50] }`. Runs `applyCreditToInvoices` + `recomputeFacilityBalances` in one transaction; busts `billing`+`bookings`. NEVER auto-applies — the operator picks where the money goes (Josh, 2026-06-26). |
...
| `POST /api/webhooks/stripe` | Stripe signature | On `checkout.session.completed`, discriminates on `metadata.type`: `portal_balance` → record `qb_payment` + auto-apply FIFO to open invoices; **`portal_prepay` / `portal_gift` (2026-06-26) → `handlePortalCredit` banks an UNAPPLIED account credit on the target resident via `createAccountCredit` (NO auto-apply, manual attribution)**; else (booking payment) set booking `payment_status='paid'` + fire receipt. Single endpoint, single `STRIPE_WEBHOOK_SECRET`. |
...
| `POST /api/debug/impersonate` | **Super admin email only** | Body `{ role: 'admin'\|'super_admin'\|'facility_staff'\|'bookkeeper'\|'stylist', facilityId, facilityName }` (`super_admin` = Franchise Admin, added 2026-06-26). Sets `__debug_role` cookie (`httpOnly: false`, sameSite lax, 8h maxAge). `getUserFacility` still normalizes the debug `super_admin`→`admin`; `isFranchiseAdmin` reads the raw cookie role for the franchise UI. |
| `POST /api/debug/reset` | **Super admin email only** | Clears `__debug_role` cookie (`maxAge: 0`). Returns `{ data: { ok: true } }`. |
| `POST /api/debug/setup-demo-franchise` | **Super admin email only** (2026-06-26) | One-click demo franchise to preview `/franchise`. Body `{ teardown? }`. Setup (idempotent): creates 2 `is_demo` facilities (FDEMO1 "Symphony Manor (Demo)" + FDEMO2 "Sunrise of Bethesda (Demo)" with sample outstanding), `seedFacilityDemoData` each, creates "Demo Franchise" + `franchise_facilities`, upserts master as `super_admin` on both, sets `__debug_role` to franchise admin on FDEMO1 → debug tab redirects to `/franchise`. teardown deletes the franchise + super_admin rows + deactivates the facilities. The `/franchise` dashboard is demo-aware (`eq(isDemo, demoFranchise)`) so demo MTD tiles populate. |
| `POST /api/debug/portal-session` | **Super admin email only** (2026-06-26) | Fake family-portal login for debug. Body `{ facilityId }`. `seedFacilityDemoData` → find/create the `demo-poa@example.com` `portal_accounts` row → link the facility's demo resident(s) → `createPortalSession` + `setPortalSessionCookie`. Returns `{ facilityCode }`; the debug tab then opens `/family/[code]` already logged in (demo balance/invoice/appointment, no magic link). `__portal_session` is independent of the master's Supabase session. |
...
| `POST /api/compliance/upload` | Authenticated; admin or stylist-owner | Multipart upload (`file`, `stylistId`, `documentType`, `expiresAt?`). `documentType` ∈ `license \| insurance \| background_check` (2026-06-26: `w9` + `contractor_agreement` removed from the stylist view — `DOCUMENT_TYPES` rejects them; label/badge maps keep the keys so legacy rows still render). Caps: 10 MB, PDF/JPEG/PNG. Uploads to private `compliance-docs` bucket at `{facilityId}/{stylistId}/{type}-{ts}.{ext}` and inserts the row with `file_url = path`. |
...
| `GET /api/feedback` | **Master admin** | 200 most recent submissions, batch-resolved sender/facility names, includes `meta` + reply fields (`reply`, `repliedAt`, `replyReadAt` — P37). |
...
| `PATCH /api/feedback/[id]` | **Master admin** (P37 2026-07-21) | Body `{ status?: new\|reviewed\|resolved, reply?: 1–2000 }` (≥1 required). Reply stamps `reply/replied_at/replied_by` + resets `reply_read_at`, then fire-and-forget: `notifyUser` (`feedback_reply` type, bell + push, url `/my-feedback`) + `buildFeedbackReplyEmailHtml` email to the submitter's profile email. Columns from `drizzle/0030_feedback_reply.sql` (self-bootstrapped in feedback-ddl). Submitter surface: **`/my-feedback`** page (all roles) lists own submissions + replies, marks unread replies read on render; linked from the feedback widget footer. Master UI: per-row "Reply…" composer in `/master-admin/feedback` (reply to a `new` item auto-flips `reviewed`; ✓ seen receipt from `replyReadAt`). |
...
| `POST /api/ai/assistant` | Any authed role (P38 2026-07-21; expanded P40 2026-07-22) | The AI personal assistant — the ONLY AI endpoint since P40 (`/api/ai/analyst` deleted). Body `{ message 1–600, history [{role:'user'\|'model', text ≤1500}] ≤10, model 'fast'\|'smart' (P42), page ≤100 (P46 — validated pathname) }`. **Response is NDJSON since P46**: `{type:'status',label}` lines while tools run, then one `{type:'done',data:{answer,pendingAction?,guide?}}` or `{type:'error',error}`; pre-stream failures stay plain JSON non-200. Server builds an `AssistantCtx` (plain master honors `selected_facility_id` directly; stylist identity via `getEffectiveStylistId`) and runs a Gemini function-calling loop (`src/lib/ai-assistant/gemini.ts`, ≤6 tool rounds + forced-text round; preamble carries a generated per-role capability line) over the ROLE-FILTERED 37-tool registry (`tools.ts` — P41-P46 added explain_feature, memory tools, start_guided_walk, create_sign/statement, switch_facility, get_rebooking_candidates, get_schedule_gaps). READS execute server-side: schedule, find_resident (enriched: last visit, 90d no-shows, live owed for billing roles), services, find_open_slots (P39), business/facility numbers (packs from ai-analyst.ts — live per-resident balance SUMs since P40), my earnings, resident ledger, stylist info, time-off requests, waitlist, signup queue, payroll summary, feedback inbox (master). WRITES only PROPOSE (17 `AssistantActionKind`s incl. switch_facility: book/cancel/reschedule/update_appointment/create_resident/update_resident/set_stylist_hours/add_time_off/decide_time_off/add_to_waitlist/add_signup_entry/create_service/update_service/update_stylist/reply_to_feedback/send_receipt) — response `{ data: { answer, pendingAction? } }` where `pendingAction.request` is server-built from resolved entities with a closed per-kind field set and executed by the CLIENT against the existing REST endpoints after the user confirms, validated against the shared `ACTION_RULES` allowlist (`src/lib/ai-assistant/action-allowlist.ts`). Bucket `aiAssistant` 60/h/user; `maxDuration 60`; 503 without GEMINI_API_KEY. UI: `<AssistantWidget>` in `(protected)/layout.tsx` (all roles, voice) + inline `<AssistantCard>` on /analytics and master-admin Reports. Harness: `npx tsx scripts/test-ai-assistant.ts` (126 checks). |
| `POST /api/ai/assistant/feedback` | Any authed role (P46 2026-07-23) | Answer thumbs. Body `{ rating 'up'\|'down', answer ≤600, question ≤600? }` → inserts into `feedback_submissions` (category 'other', `meta.source: 'assistant-thumbs'`, pagePath '/assistant'; 👎 → status 'new', 👍 → 'reviewed'). Bucket `feedback`. |
...
| 5 | Customer Balance Detail | `POST /api/super-admin/qb-import/unapplied` | Extracts payments QB received but never applied to an invoice (non-Invoice rows with nonzero open balance in the hierarchical facility → resident-sub-section report). Wipes + replaces the `qb_unapplied_credits` snapshot table (`drizzle/0008_qb_unapplied_credits.sql`; route self-bootstraps the DDL). Resident matching: exact `FXXX:subName` qbCustomerId → facility-scoped suffix. Checklist UI at `/master-admin/unapplied-credits` (server page + `unapplied-client.tsx`) — credits can be applied to open invoices ON THE SITE: `POST /api/super-admin/unapplied-credits/auto-match` (exact-amount or oldest-first FIFO-exact resident matches; preview then apply, matches re-derived server-side on apply) and `POST .../apply` (manual invoice picker via `GET .../invoices?creditId=`). Shared engine `src/lib/unapplied-apply.ts`; `applied_cents/applied_at/applied_by/applied_detail` columns (`drizzle/0009_unapplied_apply.sql`, self-bootstrapped by `src/lib/unapplied-ddl.ts`). Site-only — mirror in QB or the next Step 2 import reverts it; re-imports preserve site-applied rows by identity key. Bucket `billingImport`. |
...
**Account-credit primitive (2026-06-26)** — `src/lib/account-credits.ts::createAccountCredit({ facilityId, residentId, amountCents, source, num? })` inserts a `qb_unapplied_credits` row (`txnType` = `'Prepayment'|'Gift'|'Coupon'`, `openBalanceCents = amountCents`, `appliedCents = 0`; synthetic `qb_customer_id = RES-<id8>` when the resident lacks one). This is the SINGLE money primitive for portal prepayments, gifts, and (future) coupon credits — all become unapplied credits that an admin/bookkeeper attributes to chosen invoices via `applyCreditToInvoices`. The resident ledger (`/residents/[id]`) and `POST /api/residents/[id]/credits/[creditId]/apply` give a facility-side apply UI (previously credit application was master-only at `/master-admin/unapplied-credits`).

**Why site outstanding ≠ QB dashboard numbers** (reconciled 2026-06-12 to the penny): the QB home-page "Invoices Unpaid" widget is **last-365-days only**; site outstanding = ALL open invoices (the file's grand total); QB **A/R** = open invoices **minus unapplied credits** (~$75k as of import). Step 5 surfaces the unapplied credits; applying them in QB + re-running Step 2 converges the numbers.
...
### Send-confirmation gate (SHIPPED 2026-06-17)

Every **one-click** email/text send is gated by an "Are you sure you want to send this?" dialog (recipient + channel + one-line summary) so a stray click can't fire a real message. Reusable `src/components/ui/send-confirm-dialog.tsx` exports **`useSendConfirm()`** → `{ confirmSend, dialog }` (`confirmSend(opts): Promise<boolean>`; resolver in a `useRef` to avoid StrictMode double-fire) and **`SendConfirmDialog`** (desktop `Modal` / mobile `BottomSheet`). `SendConfirmOptions = { channel: 'email'|'sms'|'both', recipient, summary, warning?, confirmLabel? }`. Handler pattern: `if (!(await confirmSend({...}))) return` at the top, render `{dialog}` once. **7 call sites**: booking-modal Send Receipt, resident-detail Send Link, team-section Resend invite, stylist-detail Send account invite, billing-client facility Send Statement, ip-view per-resident Send + Send All (replaced `window.confirm`). For per-resident billing sends, the known `lastSentAt` is shown as the `warning` line and the send goes out with `force:true` (bypasses `SendDedupModal`); the facility statement keeps `force:false` so `SendDedupModal` remains the second layer. **Excluded** (already deliberate): Day Log Email modal, new teammate-invite form; "Copy Link" sends nothing. New one-click send buttons MUST adopt this gate (CLAUDE.md rule).
...
## P46 — Best-in-class assistant: streaming, persistence, page awareness, growth tools (2026-07-23)

- **Streaming**: `POST /api/ai/assistant` responds NDJSON — `{type:'status',
  label}` lines per tool dispatch (labels from `status-labels.ts`; every
  tool must be registered there), then one `{type:'done', data}` or
...
- **Thumbs**: `POST /api/ai/assistant/feedback` `{rating up|down, answer,
  question?}` → `feedback_submissions` (meta.source 'assistant-thumbs';
  👎 status 'new', 👍 'reviewed'). Bucket `feedback`.
- **Growth tools**: `get_rebooking_candidates` (admin/staff/master —
  getDueForVisit cadence SQL + usual service) and `get_schedule_gaps`
  (admin/staff/master; stylist = self — per-stylist idle blocks via
