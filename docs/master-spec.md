...
- **`POST /api/super-admin/import-facilities-csv`** — master admin only, rate-limited under `billingImport` bucket (5/hr), `maxDuration=60`. Accepts multipart `csv` field. **Fixed column positions** (no header detection): col[0]=name, col[1]=F-code (`/^F\d{2,4}$/`), col[3]=contact email, col[4]=billing type, col[5]=rev share %. Matches rows by `facilityCode` (col[1]) via `Map<facilityCode, facility>` pre-loaded from DB — O(1) exact match; does NOT use fuzzy name matching. Skips rows where col[3] contains "@" (non-email context), "is now", or "see above". Per matched facility: fills `contactEmail` only when currently null (col[3] when it contains "@"), always overwrites `paymentType` via `mapBillingType()` (IP+F→hybrid, IP/IPM→ip, F/NB/SC→rfms), always overwrites `revSharePercentage` (0–100). Returns `{ updated, skipped, emailsFilled, revShareSet, warnings }`. Never creates new facilities.
