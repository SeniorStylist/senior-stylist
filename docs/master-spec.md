...
| `POST /api/log/ocr` | Authenticated | Accept `images[]`, extract `{ date, stylistName, entries[] }` per sheet via Gemini 2.5 Flash, return `{ data: { sheets } }`. Each entry includes `additionalServices: string[]` when the handwriting lists multiple services joined by `+`/`/`/`&`/`and`/`,`. `maxDuration = 120`. Client batches files in chunks of 3 and sends sequential requests; sheets are merged client-side. Client renders PDF previews via `renderPdfPage(file, scale)`: scale 0.5 for upload thumbnails, scale 1.5 for review/lightbox. Both stored in parallel state arrays. While `scanning === true`, upload step shows animated loading overlay (pulsing doc SVG + bouncing sparkle, rotating `SCAN_TIPS` every 3s with fade, progress bar from `scanProgress`). Review step service fields are `<select>` dropdowns pre-populated with existing services via `fuzzyBestMatch(services, name, 0.7)`; `__new__${name}` option creates a new service. |
