...
| `GET /api/log/import-batches` | canScanLogs + master | Log Sheet History list (the "Sheets" button on /log → `LogSheetsModal`). `sourceType='ocr_scan'` batches, newest 200: master → all; bookkeeper → own uploads (cross-facility); admin/staff → own facility. Rows include facility name/code + uploader name + `deletedAt` ("Rolled back" badge). |
