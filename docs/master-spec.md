...
- **Brand alignment**: Portal (`src/app/(resident)/`) uses burgundy `#8B2E4A` with warm blush `#FDF8F8` background matching seniorstylist.com marketing site. `--color-primary` in globals.css is `#8B2E4A` (affects FullCalendar toolbar only). Admin app components (`button.tsx`, `input.tsx`, calendar, booking modal) still use teal `#0D7377` hardcoded. Entry pages (onboarding, invite/accept, unauthorized) use burgundy CTAs.
