...
| `POST /api/services/parse-pdf` | Admin | Extract services from a price sheet using **Gemini 2.5 Flash** (`temperature: 0`, JSON mode). Accepts EITHER `file` (a PDF, sent as native `application/pdf` inlineData) OR `gridText` (a spreadsheet's full cell grid as tab-separated text — `xlsx`/`xls`/`csv`, all sheets). No regex/column-mapping — Gemini reads the layout and ignores section headers, prose, dates, page markers, and blank rows. `gridText` capped at 400k chars. Returns `name, priceCents, durationMinutes, category, color, pricingType, addonAmountCents, pricingTiers, pricingOptions`. **Spreadsheets route through the SAME parser as PDFs** (client converts to grid text in `import-client.tsx` via `spreadsheetToGridText`); the old naive `parseSpreadsheetNaive` column-mapping remains only as a fallback when Gemini is unavailable. Switched off naive column-mapping because real facility price sheets are free-form (irregular columns, section headers, policy prose) and mapped to garbage rows |
