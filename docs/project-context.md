...
- QB token encryption shipped (2026-04-20): `qb_access_token` and `qb_refresh_token` are now stored as AES-256-GCM ciphertext in the DB. New `src/lib/token-crypto.ts` provides `encryptToken()`/`decryptToken()`. Key: `QB_TOKEN_SECRET` env var (32-byte hex). Encrypt/decrypt called only inside `src/lib/quickbooks.ts` — never in route handlers. `callback/route.ts` stores `encryptToken(tokens.accessToken/refreshToken)`; `doRefresh()` decrypts on read and encrypts on write; `revokeQBToken()` decrypts before sending to Intuit.
