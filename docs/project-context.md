...
- QuickBooks customer import shipped (2026-04-20): bulk-create facilities and residents from a QB XLS/XLSX customer export. New `qb_customer_id` column on both `facilities` and `residents` tables (nullable text, partial unique index). `POST /api/super-admin/import-quickbooks` (master admin only, `maxDuration=120`) parses QB Name column: `F120 - Facility Name` rows → facility upserts (with `facility_users` admin row + `franchise_facilities` link); `F120:LastName, FirstName - 291` rows → resident upserts (name flipped, room parsed, `onConflictDoUpdate` on `(name, facility_id)` to link existing manual-entry residents). `rate-limit.ts` gets new `qbImport` bucket (5/hr). UI page `/super-admin/import-quickbooks` (3 states: upload card → full-screen loading overlay with two-phase progress bar → results summary with facilities/residents counts + warnings collapsible). "Import from QuickBooks" link added to super-admin page header.
