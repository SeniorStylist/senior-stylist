...
Phase 9 Prompts 1–3 shipped (2026-04-15). Next steps:
1. **Phase 9 Prompt 4** — final polish prompt per the Phase 9 spec (backfill + any remaining UI/data cleanup). Run the backfill script that creates a `stylist_facility_assignments` row for every existing `stylists.facility_id` (not null) and populates `commission_percent` from `stylists.commission_percent`. Delete after running.
...
4. Onboard Symphony Manor + Sunrise Bethesda — invite real stylists, upload compliance docs, confirm weekly availability.
