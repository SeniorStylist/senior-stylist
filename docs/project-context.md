...
### Working
- Phase 11D.6 fixes shipped (2026-04-21): three UI/UX improvements. (1) **Facilities CSV import** — new `POST /api/super-admin/import-facilities-csv` route + `/super-admin/import-facilities-csv` page (same 3-state pattern as billing history import). Fuzzy-matches rows by facility name, fills contactEmail if null, always overwrites paymentType + revSharePercentage. "Update Facilities" header link added to Super Admin. New `rev_share_percentage integer` column on `facilities`. (2) **Inference status message** — when resident-name inference runs but fails, modal now shows amber "Could not auto-match from N resident names — please select facility manually" below the facility combobox. FacilityMatch gains `inferenceAttempted` + `inferenceResidentCount` fields. (3) **Payment total row** — new summary row above buttons in scan modal showing "Check: $X = Total Received: $X"; expands to show cash term when cashEnabled.
