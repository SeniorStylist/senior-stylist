...
- **Contact section** (below license, admin-only, display-only): shown when any of phone/address/paymentMethod is set. Phone renders as `<a href="tel:...">` in burgundy. Address as `text-sm text-stone-600`. Payment method as a `bg-stone-100 text-stone-700` pill badge. Section label: "Contact" (`text-xs font-semibold text-stone-500 uppercase tracking-wide`).
