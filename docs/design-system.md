...
- **Clickable date in calendar header**: `showCalDatePicker` boolean state (NOT `showPicker()` — iOS Safari doesn't support it). Button toggles state; when true, renders `<input type="date" autoFocus>` absolutely positioned below the button (`absolute top-full left-0 mt-1 z-50 rounded-xl border border-stone-200 shadow-lg bg-white px-3 py-2 text-sm`). `onChange` calls `gotoDateRef.current?.(new Date(e.target.value + 'T12:00:00'))` and closes picker. `onBlur` closes picker. Input `value` is `calendarStartDate` state, updated on each `datesSet` callback. `gotoDateRef` wired to `fcRef.current?.getApi().gotoDate(date)` in CalendarView's `useEffect`. Inline SVG calendar icon (not lucide-react).
- **Clickable date in daily log header**: same state-based pattern. `showLogDatePicker` boolean state. Input positioned `absolute top-full left-1/2 -translate-x-1/2`. `onChange` calls `navigateDate(e.target.value)` and closes picker.
- **NEVER use `input.showPicker()`** for date navigation — not supported on iOS Safari. Always use the state-toggle visible-input pattern above.
...
`getiOSUIVariant() → iOSUIVariant` — maps version to `'ios26+'` (≥26), `'ios16-18'` (16–25), `'ios-old'` (15 and below), or `'ios-unknown'`.

`detectAndroidBrowser() → AndroidBrowser` — detects `'android-chrome' | 'android-samsung' | 'android-firefox' | 'android-edge' | 'android-other'` from UA.
...
**Other device guides:**
- `ios-chrome` — amber warning + 2 steps: tap "aA" → "Open in Safari"
- `android-chrome` — if `deferredPrompt`: single Install App button; else 3-step manual (⋮ menu → "Add to Home screen" → Add)
- `android-samsung` — 3 steps: ≡ bottom-right → "Add page to Home screen" → Add
- `android-firefox` — 3 steps: ⋮ menu → "Install" → "Add automatically"
- `android-edge` — 3 steps: ⋯ menu → "Add to phone → Home screen" → Add
- generic fallback

Dot progress: active dot `width: 20px`, inactive `8px`, all `h-2 rounded-full`, tappable. Final step button: "Done! 🎉". All mockups are inline SVG + CSS divs. Step numbers: `w-7 h-7 rounded-full bg-[#8B2E4A] text-white`. Bouncing arrows: `animate-bounce` in burgundy.
...
### Settings Shell + Section Header (2026-06-15)
- **Two-pane settings shell** (`settings-client.tsx`): the left rail groups categories under small uppercase group labels (`text-[10.5px] font-semibold text-stone-400 uppercase tracking-wide`), each nav row = lucide icon (17px) + label + optional count badge. Active row is burgundy-tinted (`bg-[#F9EFF2] text-[#8B2E4A] font-semibold`) with the icon recolored to `#8B2E4A`; inactive icons are `text-stone-400`. Group headers render only when there's more than one visible category (single-category roles get a flat list). A rail search box (lucide `Search` + `focus:shadow-[0_0_0_3px_rgba(139,46,74,0.08)]` glow) appears when there are >4 categories; typing flattens the grouped list.
- **Section header pattern** (reusable beyond settings): a page/section opens with a `flex items-start gap-3` header — a `w-10 h-10 rounded-xl bg-[#F9EFF2] text-[#8B2E4A]` icon chip + `text-lg font-semibold text-stone-900` title + `text-sm text-stone-500` one-line description. This is the canonical "what is this screen" anchor; drive the icon + description from a single metadata map so the nav row and the header never drift. When the shell renders this header, the child section component must NOT render its own duplicate top-level title.
- **`<PageHeader>` primitive** (`src/components/ui/page-header.tsx`, 2026-06-15) — the page-level version of the section header: a `w-11 h-11 rounded-2xl bg-[#F9EFF2] text-[#8B2E4A]` icon chip (22px lucide icon) + the DM Serif Display `<h1>` (`text-2xl font-normal`, the established page-title look) + an optional `subtitle` ReactNode (`text-sm text-stone-500`). Props: `icon` (LucideIcon), `title`, `subtitle?`, `className?`, `data-tour?`. It renders ONLY the left block — keep each page's existing `flex justify-between` wrapper and right-side actions row untouched (drop `<PageHeader>` in where the old `<div><h1/><p/></div>` text block was). **All top-level nav destinations now use it** — Residents (`Users`), Services (`Sparkles`), Stylist Directory (`Scissors`), Billing (`CreditCard`), Reports/Analytics (`BarChart3`), Payroll (`Wallet`), Master Admin (`Shield`), My Account (`UserRound`). Settings uses the in-shell section-header variant above. NOT used on the Daily Log (specialized centered date-stepper header) or entity-detail pages (resident/stylist detail lead with an avatar, not an icon chip). When adding a new top-level page, use `<PageHeader>` with a distinct lucide icon.

### Payments — Card-On-File UI (2026-06-28)
- **Stripe Elements components** live in `src/components/payments/`: `<AddCardForm>` (SetupIntent + Payment Element, save-card consent copy), `<SavedCardsCard>` (list/add/remove cards — `role` prop gates billing roles; omit `role` for the family-portal context), `<CofPanel>` (resident auto-pay toggle + method select + Collect-now + Send-pay-link + Take-card-payment, billing-role gated), `<TakePaymentModal>` (two-step: amount + save-card → Payment Element `confirmPayment`). Shared `stripe-browser.ts::getStripePromise(key)` caches the Stripe.js singleton per publishable key — always reuse it, never call `loadStripe` ad-hoc.
- **Card brand chips**: `capitalize` brand + `•••• {last4}` + `exp MM/YY`; default card gets an emerald capsule (`bg-emerald-50 text-emerald-700 border border-emerald-200`).
...
### Polish sweep patterns (2026-07-03)
- **Optimistic + snapshot rollback** is the canonical mutation pattern for instant-feel actions: `const snapshot = state` → apply optimistic state + close the form → fetch → on `!ok` restore the snapshot, restore/reopen the form, and show the REAL error (`firstErrorMessage`-style), never a generic "failed". Temp inserts use an `optimistic-{Date.now()}` id reconciled to the server id on success (see residents `handleAdd`).
