...
- OCR log import: POST /api/log/ocr accepts multipart/form-data `images[]` (plural, multiple files allowed). Uses `@google/generative-ai` SDK (v1beta endpoint) with model **`gemini-1.5-flash-8b`** — NEVER use `gemini-2.0-flash` (silently fails) or `gemini-1.5-flash` (returns 404 on v1beta despite being valid). `gemini-1.5-flash-8b` is the confirmed working model. Supports JPEG, PNG, WEBP, GIF, and **PDF** — Gemini handles PDFs natively via `inlineData` with `mimeType: 'application/pdf'`, no conversion needed. Processes files sequentially with a 45s per-file `Promise.race` timeout guard. Extracts `{ date, stylistName, entries[] }` per sheet. Returns `{ data: { sheets: [{ imageIndex, date, stylistName, entries }] } }`. Per-image errors are included inline rather than aborting the whole request. GEMINI_API_KEY is checked server-side before use (returns 500 if missing). Route exports `maxDuration = 60` and `dynamic = 'force-dynamic'` for Vercel timeout handling.
