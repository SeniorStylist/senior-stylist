...
- **`import-facilities-csv` route uses fixed column positions** — col[0]=name, col[1]=F-code (`/^F\d{2,4}$/`), col[3]=email, col[4]=billing type, col[5]=rev share %. Matches by `facilityCode` exact key lookup (`Map<string, facility>`) NOT fuzzy name. Skips junk rows where col[3] contains "@" (non-email context), "is now", or "see above". `mapBillingType()` maps: IP+F→hybrid, IP/IPM/IP*→ip, F/NB/SC/F*→rfms.
