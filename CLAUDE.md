...
- OCR log import: POST /api/log/ocr accepts multipart/form-data `images[]` (plural, multiple files allowed). Calls Google Gemini **1.5 Flash** (`gemini-1.5-flash`) via `GEMINI_API_KEY` (server-side only) — NEVER use `gemini-2.0-flash`, it is not available and silently fails. Supports JPEG, PNG, WEBP, GIF, and **PDF** — Gemini handles PDFs natively via `inlineData` with `mimeType: 'application/pdf'`, no conversion needed. Processes files sequentially. Extracts `{ date, stylistName, entries[] }` per sheet. Returns `{ data: { sheets: [{ imageIndex, date, stylistName, entries }] } }`. Per-image errors are included inline rather than aborting the whole request. GEMINI_API_KEY is checked server-side before use (returns 500 if missing).
