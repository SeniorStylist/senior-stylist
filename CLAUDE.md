...
- **Help Center / Driver.js tours (Phase 12)** — guided tours target real elements via `[data-tour="X"]` (desktop) and `[data-tour-mobile="X"]` (mobile-only elements like bottom-tab MobileNav and mobile-only buttons in `log-client.tsx`). The `tourSelector()` helper in `src/lib/help/tours.ts` picks per-viewport at tour-start. **Never use the same `data-tour` value on both desktop and mobile elements** — Driver.js's `querySelector` picks first match and gives unpredictable results. Mobile-only nav buttons get `data-tour-mobile`; desktop-only sidebar nav gets `data-tour`; shared elements (residents-search, settings-facility-form) only need `data-tour`. Tour buttons use burgundy `#8B2E4A` (NOT teal). `driver.js` is dynamic-imported inside `startTour()` so it stays out of the global bundle. Tour steps live in `TOUR_DEFINITIONS` map with separate `desktop`/`mobile` `DriveStep[]` arrays. Tutorial catalog lives in `TUTORIAL_CATALOG` array; cards with `tourId: null` show "Coming soon" tooltip on the Guided Tour button. CSS overrides for `.senior-stylist-tour` popover live at the bottom of `globals.css`. **`profiles.has_seen_onboarding_tour`** (boolean default false NOT NULL) drives the first-login welcome modal — flag flips via `POST /api/profile/onboarding-seen`. **`<HelpTip tourId label description />`** in `src/components/ui/help-tip.tsx` is the canonical contextual `?` icon — desktop opens click-outside popover, mobile opens existing `<BottomSheet>`. Currently injected at: daily log date header, dashboard calendar header, residents new-button, billing Outstanding tile, settings Working Hours, settings QuickBooks header.
