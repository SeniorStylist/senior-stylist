...
- **Renderer**: four absolutely-positioned `bg-black/60 z-[200]` panels surround the spotlight rectangle (target's `getBoundingClientRect()` + 8px padding). A separate `rounded-2xl pointer-events-none z-[201]` div is the visual ring. Bottom sheet card is `z-[202] bg-white rounded-t-3xl` with `env(safe-area-inset-bottom)` padding. The four panels block clicks on non-spotlight UI; the spotlight area passes clicks through to the underlying element naturally (no panel there); the ring is `pointer-events: none`.
- **Premium polish (2026-05-31)**: the four panels are driven by a **JS-lerp RAF loop** (`lerp(cur, target, 0.25)` until within 0.5px, ported from the scripted `spotlight-mask.tsx`) — the panels no longer carry `transition-all duration-200`, so the spotlight *glides* between steps instead of snapping. The geometry is held in `animRef` + `animRect` state; first paint of a run jumps straight to the target (no glide-in from 0,0) via `animInitRef`, reset when `step` clears. The ring uses the scripted premium box-shadow — info steps get `0 0 0 3px rgba(139,46,74,0.55)`, action steps get the bright white inner highlight + burgundy glow (`0 0 0 3px rgba(255,255,255,0.95), 0 0 0 7px rgba(139,46,74,0.75), 0 0 22px 6px rgba(139,46,74,0.45)`) plus the existing `mobile-tour-spotlight-pulse`. The sheet's title/description block is keyed on `stepIndex` with a `mobile-tour-step-enter` crossfade (3px translateY, 0.14s, reduced-motion-guarded). **Never re-add `transition-all` to the panels** — it fights the RAF loop and reintroduces the snap.
...
**Phase 12Q — Tour Completion Tracking**:
- **`profiles.completed_tours text[] NOT NULL DEFAULT '{}'`** — Drizzle column: `text('completed_tours').array().notNull().default(sql\`'{}'\`)`. Added after `hasSeenOnboardingTour`.
- **`POST /api/profile/complete-tour`** — validates `tourId` against `TOUR_DEFINITIONS`, idempotent `array_append` via `db.execute(sql\`UPDATE profiles SET completed_tours = array_append(...) WHERE id = $userId AND NOT ($tourId = ANY(completed_tours))\`)`. Auth-gated, rate-limited under `completeTour` bucket (20/hr/user). Uses Drizzle `db.execute` NOT a Supabase RPC — the project's service-role key makes `auth.uid()` null inside SECURITY DEFINER RPCs.
- **Completion fires at the terminal step, AFTER `setTourModeActive(false)`** — both engines dispatch `window.dispatchEvent(new CustomEvent('tour-completed', { detail: { tourId } }))` then call `fetch('/api/profile/complete-tour', ...).catch(() => {})`. The ordering is critical: the Phase 12O interceptor blocks `/api/*` POSTs while tour mode is active; firing before the flag flips would silently discard the request.
- **`tour-completed` CustomEvent** enables the `/help` page to show the badge instantly (optimistic), before the server round-trip completes. `help-client.tsx` subscribes in a `useEffect` and merges new IDs into `localCompleted: string[]` state initialized from the server-fetched prop.
- **`/help` page** queries `profiles.completedTours` via `db.query.profiles.findFirst({ columns: { completedTours: true } })` and passes it as `completedTours: string[]` to `<HelpClient />`.
- **`✓ Done` badge** on `<TutorialCard />` — `completed` prop (boolean); emerald capsule `bg-emerald-50 text-emerald-700 border border-emerald-200`. Never shown for Coming Soon cards (`tourId = null`) because `localCompleted.includes(null ?? '') === localCompleted.includes('')` is always false.

**Phase 12R — Onboarding Checklist Widget**:
- **`ONBOARDING_CHECKLIST`** exported from `src/lib/help/tours.ts` (after `TUTORIAL_CATALOG`) — `Record<string, { tourId: string; label: string }[]>`. Keys: `stylist` (4 items), `facility_staff` (3), `admin` (4), `bookkeeper` (3). `super_admin` maps to `admin` at runtime; master admin and unlisted roles get no checklist.
- **`<OnboardingChecklist />`** (`src/components/help/onboarding-checklist.tsx`) — `role`, `completedTours`, `isMaster`, `userId` props. Fixed bottom-right widget, `z-[100]` (below tour overlays). Only mounted in `DashboardClient` (`dashboard-client.tsx`), NOT in global layout. Two render paths in `DashboardClient` (mobile-stylist and desktop) each get the widget.
- **localStorage keys**: `onboardingChecklistCollapsed` (shared) and `onboardingChecklistDismissed:{userId}` (user-specific). Both initialized lazily via `useState(() => localStorage.getItem(...))` to avoid hydration flicker. `typeof window === 'undefined'` guard for SSR.
- **Auto-dismiss**: when `completedCount === totalCount`, shows "🎉 You're all set!" for 1.5s then sets the dismissed localStorage flag and unmounts. × button also sets the flag immediately. Neither path is server-side — dismissal is purely client localStorage.
- **Entrance animation**: `transform: translateY(120%) → translateY(0)` controlled by `visible` state set via 1-second `setTimeout` on mount. `transition-transform duration-300` Tailwind class drives it.
- **Live updates**: subscribes to `tour-completed` CustomEvent in a `useEffect` — items check off instantly when a tour finishes, no reload needed.
- **bottom positioning**: `style={{ bottom: 'calc(env(safe-area-inset-bottom) + 80px)' }}` — per CLAUDE.md rule for fixed bottom elements. No Tailwind `bottom-N` class on the same element.
- **`dashboard/page.tsx`** extends profile query to include `completedTours`, adds `isMaster` variable, passes `completedTours`, `isMaster`, `userId` as new props to `<DashboardClient />`.

**Desktop Driver.js premium chrome (2026-05-31)** — the legacy desktop tours are styled to match the scripted premium look entirely via the `.senior-stylist-tour` / `.driver-*` CSS block in `globals.css` (no `driverConfig` changes — `animate`/`smoothScroll` were already on): popover has a 4px burgundy `::before` accent bar + `ss-tour-popover-enter` entrance (fade + 4px translateY + scale 0.97→1) + the scripted shadow token; `.driver-highlighted-element::after` is a soft burgundy glow (`box-shadow` not `border`) with a low-amplitude `tour-element-pulse`; `.driver-stage` gets `transition: all 0.32s cubic-bezier(0.32,0.72,0,1)` for a smooth cutout glide. All covered by the universal `prefers-reduced-motion` block. The legacy `TOUR_DEFINITIONS` copy (all ~31 tours) was rewritten to the tight coaching voice (titles ≤8 words, descriptions ≤30 words, no "Let's/Go to/In a real setup"; "Tap" on mobile) — selectors/routes/`isAction`/ids untouched, so `scripts/check-tours.ts` stays 174/174.

**Phase 12P — SPA tour navigation + MutationObserver waits**:
- **Cross-route hops use `router.push()` via `getTourRouter()`**, NOT `window.location.href`. Module state survives the transition — the tour engine, `_tourModeActive` flag, fetch interceptor, and Driver.js singleton all stay live. `window.location.href` is the fallback only when `getTourRouter()` returns null (SSR race before `<TourRouterProvider />` populates the ref). In the SPA path the engine does NOT `saveSessionState` and does NOT `return` — it falls through to `waitForElement`, which resolves on the new page's render via MutationObserver. Do NOT re-add `saveSessionState` or `return` to the `router.push` branch.
- **`src/lib/help/tour-router.ts`** holds the module-level `_router` ref. `setTourRouter(router)` is called once on layout mount by `<TourRouterProvider />` (`src/components/help/tour-router-provider.tsx`, mounted inside `<ToastProvider>` next to `<TourResumer />`). `getTourRouter()` returns the current ref or null. Both engines (`tours.ts`, `mobile-tour.ts`) call `getTourRouter()` at the cross-route branch and pick `router.push()` when available.
- **`waitForElement` is MutationObserver-based**, not requestAnimationFrame polling. Returns immediately when the element is already in the DOM (microtask resolve, no rAF tick). Otherwise observes `document.body` `{ childList: true, subtree: true }` and resolves on the first match. Always disconnects on resolve OR timeout — no observer leaks. Visibility guard (`offsetParent !== null`) is preserved on both the synchronous initial check and the MutationObserver callback.
- **`<TourResumer />` is now a safety net, not the primary cross-route path**. It still fires on protected-layout mount and reads sessionStorage, but in the new SPA flow it has nothing to resume because sessionStorage was never written. It still handles: (a) manual browser refreshes mid-tour, (b) the `getTourRouter()` null fallback path, (c) external `window.location.href` writes (debug routes, etc.). Do NOT delete it.
- **Old previous-step highlight cleanup is BEFORE `router.push()`**: desktop calls `destroyActiveTour()` and mobile calls `destroyActiveMobileTour()` + `dispatchHide()` before the navigation so the old overlay clears before the new route renders. Keep this ordering.
...
**Phase 12O — Tour Mode (Demo Mode) write interception**:
- `src/lib/help/tour-mode.ts` — module-level `_tourModeActive` flag. `setTourModeActive(active)` is no-op when value is unchanged (dedupe), otherwise dispatches a `tour-mode-change` CustomEvent with `detail: { active }`. `isTourModeActive()` returns the current value. Lives outside React so both tour engines and the fetch interceptor can read it without prop drilling.
- `src/lib/help/tour-fetch-interceptor.ts::installTourFetchInterceptor()` — patches `window.fetch` once (module-level `_installed` guard; safe to call repeatedly). The patched function is a no-op pass-through when `isTourModeActive()` is false. When active: intercepts only same-origin `/api/*` writes (`POST`/`PUT`/`PATCH`/`DELETE`) and returns a smart fake `Response` (status 200, JSON body). `GET`/`HEAD` and external URLs (Supabase, Resend, Intuit OAuth) always pass through. Smart fake response shapes — order matters, more specific URLs first:
  - `POST /api/bookings/[id]/receipt` → `{ data: { emailSent: true, smsSent: false } }`
  - `POST /api/log/ocr/import` → `{ data: { created: { bookings: 0 } } }`
  - `POST /api/log/ocr` → `{ data: { sheets: [] } }`
  - `POST|PUT /api/log[/...]` → `{ data: { id: 'demo-<ts>', finalized: true, finalizedAt, notes: '' } }`
  - `POST /api/bookings` → `{ data: { id: 'demo-<ts>', status: 'scheduled' } }`
  - `PUT|PATCH /api/bookings/[id]` → `{ data: { id: 'demo-<ts>', status: 'completed', paymentStatus: 'paid', priceCents: 0 } }`
  - `* /api/residents` → `{ data: { id: 'demo-<ts>', name: 'Demo Resident' } }`
  - everything else → `{ data: {}, ok: true }`
- **Known limitation**: client code that destructures un-covered response shapes may throw (e.g. nested `data.booking.stylist.name`). Accepted tradeoff — the tour copy guides the user through the click, the tour advances regardless of the resulting DOM state. If a specific tour step blows up the UI badly enough that the user can't recover, add a route-specific shape to `buildFakeResponse()`.
- **`setTourModeActive(true)`** is called at the **top of `startTour()`** (after the mobile/desktop branch) and at the **top of `startMobileTour()`** (after the re-entry guard). The interceptor install also happens here, BEFORE the flag flip.
- **`setTourModeActive(false)` placement — exits only, NOT in `destroyActiveTour()`**. Both engines call their destroy helpers for **internal step transitions** (info next/prev, action auto-advance), so putting the flag flip inside destroy would briefly flip mode off between every step. The flag flip lives at exactly two paths per engine:
  - Desktop `tours.ts`: terminal-step branch in `runStep` (where `index >= def.steps.length`), and `onCloseClick`
  - Mobile `mobile-tour.ts`: terminal-step branch in `runMobileStep`, and `activeCloseHandler`
  - Cross-route hard-nav (desktop line ~770, mobile line ~103) does NOT need an explicit flip because `window.location.href = …` reloads the page and module state resets to false automatically; `TourResumer` re-installs + `startTour`/`startMobileTour` re-flips to true.
- **`tour-resumer.tsx` re-installs the interceptor** on layout mount (before `resumePendingTour()`). The page reload wipes module state, so the patch needs to be re-applied during the window between mount and the actual `startTour`/`startMobileTour` call. `setTourModeActive(true)` is NOT duplicated in the resumer — `startTour`/`startMobileTour` handle that.
- **`<TourModeBanner />`** (`src/components/help/tour-mode-banner.tsx`) — fixed top banner. Mounted at the very first child of `(protected)/layout.tsx`'s outer `<div>`, OUTSIDE `<main>` and OUTSIDE `<ToastProvider>` so it isn't clipped by the inner scroll container's `overflow: auto`. Listens to the `tour-mode-change` CustomEvent; renders always (never unmounted) but translates off-screen when inactive so the slide animation works on both directions. `z-[250]` (above Driver.js overlay `z-[200]` and mobile tour overlay `z-[202]`). Burgundy `bg-[#8B2E4A]/90 backdrop-blur-sm`, white 12px text "🎓 Tutorial Mode — changes won't be saved", 200ms `translateY` transition.
...
- `role 'bookkeeper'` (Phase 11J.1, expanded 2026-05-06, walk-in create added 2026-05-29) — billing + payroll + analytics + log sheet scanning + booking field edits (service/price/paymentStatus/notes/resident — NOT date/time/stylist/cancel) + **manual walk-in booking creation** (`POST /api/bookings` now allows bookkeeper — same outcome as the OCR import path they already use) + **duplicate-resident merge** (they create dupes via OCR scanning — `POST /api/residents/merge` and `GET /api/residents/duplicates` allow bookkeeper; the Duplicates button is gated on `canMergeDuplicates = canEdit || bookkeeper` in `residents-page-client.tsx`). READ-ONLY on residents otherwise (no add/edit). Sees Daily Log (scan + edit booking billing fields + Add Walk-in) / Billing / Analytics / Payroll / Settings. Home page is `/log` (redirected from `/dashboard`).
...
**Schema**: `signup_sheet_entries` table — `id`, `facility_id`, `resident_id` (nullable), `resident_name`/`room_number` (denormalized for display), `service_id` (nullable), `service_name`, `requested_time` (text 'HH:MM' tz-agnostic, nullable), `requested_date` (date), `preferred_date` (date, nullable — Phase 12S, drives auto-assignment), `notes`, `created_by`, `assigned_to_stylist_id`, `status` ('pending'|'scheduled'|'cancelled'), `booking_id` (set on convert), timestamps. Indexes: `(facility_id, requested_date)` and partial `(assigned_to_stylist_id, requested_date) WHERE status='pending'`. RLS `service_role_all`. **Naming**: column is `assigned_to_stylist_id` (Drizzle `assignedToStylistId`), NOT `assigned_stylist_id`. Do NOT add a duplicate.

**Auto-assignment** (Phase 12S — `src/lib/signup-sheet-assignment.ts::resolveAssignedStylist(facilityId, preferredDate, db)`): priority order is (1) date-aware match — facility stylists with `stylist_availability` for that day-of-week, picking the least-loaded by non-cancelled bookings on that date when multiple candidates; (2) fallback — most-recently-updated active stylist assigned to the facility; (3) null. Called from `POST /api/signup-sheet` when caller doesn't supply `assignedToStylistId`.

**API routes** (under `src/app/api/signup-sheet/`):
- `POST /` — admin/facility_staff create entry; verifies resident+service+stylist scope; auto-fills room from resident if not provided; auto-assigns stylist via `resolveAssignedStylist` when caller omits `assignedToStylistId` (Phase 12S). Accepts `preferredDate` (YYYY-MM-DD nullable).
- `GET /?date=YYYY-MM-DD` — any role at facility; stylists only see entries assigned to them OR unassigned (others see all). **Phase 12S** — supports `?scope=all` (admin/facility_staff only — returns all pending entries facility-wide; stylists get 403) and `?countOnly=true` (returns `{data:{count:n}}`, skips date filter so badge counts future requests). **Canonical filter is `status='pending'`** (not `!= 'cancelled'`) — scheduled entries are real bookings on the calendar and would inflate the badge.
- `PATCH /[id]` — admin/facility_staff edit any field; stylists may PATCH only `notes` on entries assigned to them; rejected once `status='scheduled'`. Cancellation flows through `{status:'cancelled'}` PATCH — NO DELETE route exists.
...
**UI surfaces**:
- **Facility-side**: `<SignupSheetPanel>` opened from a "Sign-Up Sheet" button in the dashboard header (admin + facility_staff only). Right-anchored modal on desktop, bottom sheet on mobile. Form fields: resident typeahead with inline create, service typeahead (no inline create), preferred time, preferred date (Phase 12S — optional, drives auto-assignment), stylist override dropdown, notes `<textarea rows={2}>` (Phase 12S — was `<input>`). Admin/facility_staff also see a **"Pending requests" section** (Phase 12S — inline `<AdminPendingSection>` defined in the same file) at the TOP of the panel showing all facility-wide pending entries (uses `?scope=all`), with per-row × buttons that PATCH `status:'cancelled'`. Collapses behind "Show all (N)" when N>5. Today's queue grouped by stylist remains below the form.
- **Stylist-side**: `<StylistPendingEntries>` collapsible amber panel above the calendar. **Phase 12S** changes: forwards a `ref` (used by `dashboard-client.tsx` for FullCalendar `Draggable` registration), accepts `facilityTimezone` and `viewAsAdmin` props. Each entry card is HTML5-draggable (`data-signup-entry-id` + `draggable={true}`) with a `<GripVertical>` drag handle (`hidden md:flex` — desktop only). Cards now show: preferred-date chip (`text-xs bg-stone-100 text-stone-600 rounded-full`, lucide `<Calendar size={12}>` icon), notes (italic gray, 80-char truncate), assigned-stylist name when `viewAsAdmin` (`text-xs text-stone-400 "→ Name"`). Convert button renamed "Pick time →".

**Drag-to-calendar** (Phase 12S): `<FullCalendar>` in `calendar-view.tsx` gets `droppable={!!onSignupDrop}` + `drop` callback that reads `arg.draggedEl.dataset.signupEntryId`. `dashboard-client.tsx` instantiates FC `Draggable` from `@fullcalendar/interaction` once per panel via `useEffect` keyed on `pendingSignups.length` (lazy-imported to keep FC out of non-stylist bundles), targeting `[data-signup-entry-id]`. The drop handler opens BookingModal pre-filled with `prefillResidentId`/`prefillServiceId`/`signupSheetEntryId` + the dropped date as `modalStart`. **Mobile** is "Pick time →" only — touch drag is flaky in FC; the drag handle is `hidden md:flex`.

**Pending count badge** (Phase 12S — `src/components/signup-sheet/pending-signup-badge.tsx`): `<PendingSignupBadge role={role} />`. Stylist-only (returns null otherwise). Fetches `GET /api/signup-sheet?countOnly=true` on mount, renders a burgundy `bg-[#8B2E4A] text-white rounded-full` pill next to the Calendar nav item. Mounted in `sidebar.tsx` (inline after the nav label) and `mobile-nav.tsx` (absolute-positioned at the top-right corner of the Calendar icon). Mirrors the `<NeedsReviewBadge />` pattern.

**BookingModal integration**: three optional props — `prefillResidentId`, `prefillServiceId`, `signupSheetEntryId`. When `signupSheetEntryId` is set, the create POST goes to `/api/signup-sheet/[id]/convert` instead of `/api/bookings`. The basePayload always includes `stylistId: pickedStylist.id` when available (the convert endpoint requires stylistId; booking POST treats it as optional).
...
**Help tours** (rewritten Phase 12S): `facility-staff-signup-sheet` (7 steps, action chain through Sign-Up Sheet button → form → preferred-date → notes → submit) and `stylist-signup-sheet` (6 steps including drag-to-calendar explainer). `ClipboardList` is the canonical icon (in `TutorialIcon` union + `ICON_MAP` in `tutorial-card.tsx`). **Data-tour anchors**: `signup-sheet-button` (sidebar entry — dashboard-client), `signup-sheet-panel` (panel root), `signup-sheet-form` (form container), `signup-sheet-preferred-date`, `signup-sheet-notes`, `signup-sheet-submit`, `stylist-pending-panel` (was `stylist-signup-sheet-panel` pre-12S — renamed), `stylist-pending-entry`, `stylist-pending-convert`.

## Stylist Check-In (Phase 12T)

- **`stylist_checkins` table** — `(stylistId, facilityId, date)` is the unique key. `delayMinutes` is computed at the time of insertion (UTC instant math: `now - firstBookingStart`); the post-flight reschedule sheet uses the SAME delay applied to all future bookings — it does NOT recompute per row.
- **Idempotency contract for `POST /api/checkin`**: re-tapping returns the original row's `delayMinutes` (NOT a recomputed value). Driven by `findFirst` inside `db.transaction` — second tap short-circuits before the INSERT.
- **`PUT /api/bookings/bulk-reschedule`** is **stylist-only**. Body is `{ bookingIds[], shiftMinutes }`. The route fetches all rows in one query, validates each (ownership, facility, status ∉ {cancelled, completed}, date == today in facility tz), then bulk-shifts inside `db.transaction()`. Any single failure aborts the whole transaction with a 422 + invalid list. **No Google Calendar sync** — accepted tradeoff; document if real-world testing surfaces desync issues.
- **Banner visibility (in `<CheckInBanner>`)**: returns null when `role !== 'stylist'` OR `alreadyCheckedIn` OR `todayBookings.length === 0` OR `now > lastBookingEnd`. All checks are pure client-side (no useEffect/state for the time check — it's just `Date.now()` at render time).
- **Prop naming collision**: `dashboard-client.tsx` ALREADY has a local `const todayBookings` (line ~462) and a local `const todayDateStr = useMemo()` (line ~229). The new server-passed prop for check-in is therefore named `checkinTodayBookings` to avoid the duplicate-identifier TS error. Do NOT rename to `todayBookings` — collision will recur. The date `todayDateStr` is NOT passed as a prop; the existing local useMemo serves both surfaces (same value, same `getLocalParts` helper).
- **`dayRangeInTimezone(dateStr, tz, dayShift?)`** lives in `src/lib/time.ts`. Extracted from `reconciliation.ts` during 12T so the check-in route can share it. Returns `{ start: Date; end: Date } | null`. Use this whenever you need a `[startUtc, endUtc)` window for a calendar date in a specific facility's timezone.
- **Tour anchors**: `data-tour="checkin-banner"` on the banner div, `data-tour="checkin-button"` on the "I'm Here →" button. Keep both — `scripts/check-tours.ts` validates them.
- **Rate-limit buckets**: `checkin: 10/h/user`, `bulkReschedule: 20/h/user`. Both keyed on `user.id`.

---

## Command Palette (Phase 12V)

- **`GET /api/search?q=<2-100 chars>`** — auth-gated, **admin/bookkeeper/master only** (403 for stylist/facility_staff). Master admin (env-email match) bypasses facility scoping. Rate-limited via `search` bucket (60/min/user, keyed on `user.id`). Returns `{ data: { residents: [...], stylists: [...] } }` with max 8 each. Residents: inner-joined to facilities so `facilityName` is always populated. Stylists: for admin/bookkeeper, INNER JOIN `stylist_facility_assignments` filtered to the caller's facility (uses `selectDistinct` to dedupe in case of multi-facility assignments); for master, `LEFT JOIN facilities` on `stylists.facilityId` (the home facility — may be null for franchise-pool stylists; we don't enumerate every assignment).
- **CustomEvent `'open-command-palette'`** — fired by the sidebar search button, listened-for by `<CommandPalette />`. New event name; no collisions with existing tour events (`tour-completed`, `tour-mode-change`, `help-mobile-tour-*`).
- **`<CommandPalette />`** (`src/components/command-palette/command-palette.tsx`) — mounted in `(protected)/layout.tsx` inside `<ToastProvider>` next to the tour providers. Role-gated at mount site: only renders when `activeRole === 'admin' || activeRole === 'bookkeeper' || isMaster` (super_admin is normalized to `'admin'` by `getUserFacility()` and gets it automatically). Stylist/facility_staff: component is not mounted, no keyboard listener, no API requests possible.
- **Keyboard**: CMD+K / CTRL+K toggles open/closed; Esc closes. Input is focused via `requestAnimationFrame` on open. Arrow keys move `activeIndex` across the flat list (pages → residents → stylists). Enter selects via `router.push(item.route)` (SPA navigation, not `window.location.href`).
- **Pages list** (`src/lib/command-palette-pages.ts::PALETTE_ROUTES`) is filtered client-side by `isMaster || roles.includes(role)`. Master-only entries (Master Admin, Stylist Directory) have `roles: []` so only master sees them. Page icons resolved via local `PAGE_ICON_MAP` in the component (NOT the help system's `ICON_MAP` — they're separate; the help system carries `Search` for the tour catalog, the palette carries page icons for results).
- **Sidebar search trigger** — inline SVG (sidebar uses SVG icons, not lucide). Lives at the top of the `<nav>` element (line ~388 in `sidebar.tsx`), gated on `role === 'admin' || role === 'bookkeeper'` (`super_admin` normalized to `admin` covers master too). `hidden md:flex` — desktop only. `data-tour="cmd-k-trigger"` anchor.
- **Tour**: `admin-command-palette` (4 steps, `desktopOnly: true`) — sidebar trigger highlight + 3 info popovers. NOT added to `ONBOARDING_CHECKLIST` — it's a discovery feature, not a must-learn first-day item.
- **No mobile trigger** — keyboard shortcut still works on tablet/iPad with external keyboard, but no on-screen trigger below md breakpoint. Phase 12W (Peek Drawer) will be the mobile counterpart for jump-to-resident.

---

## Peek Drawer (Phase 12W)

- **`GET /api/peek?type=resident|stylist&id=<uuid>`** — auth-gated, all roles except `viewer` (403). Rate-limited via `peek` bucket (120/min/user, keyed on `user.id`). Master admin (env-email match) can peek any resident/stylist; all other roles are scoped to their `facilityUser.facilityId`. **Cross-facility peeks return 404 (not 403)** to avoid leaking resident/stylist existence. Stylist scope uses an `INNER JOIN stylistFacilityAssignments` (filtered `eq(facilityId, callerFacilityId), eq(active, true)`) — handles franchise-pool stylists whose `stylists.facility_id` is null.
- **`residents` table has no `facility` relation** in `residentsRelations` (only `bookings`). The peek route does a separate `db.query.facilities.findFirst({ where: eq(facilities.id, resident.facilityId) })` rather than adding a new relation. Stylist branch reuses the `stylistFacility` join already inline.
- **Booking queries always filter `eq(bookings.active, true)`** (non-negotiable). Last 3 completed visits: `status='completed', active=true` ordered `startTime DESC`. Next 1 upcoming: `status='scheduled', active=true, startTime > now()`. Service name fallback chain: `service?.name ?? rawServiceName ?? 'Unknown service'` (Phase 12B null-service guard).
- **Stylist booking counts** (`todayCount`, `weekCount`): two `db.execute(sql\`...\`)` queries — `(stylist_id, facility_id, active=true, status != 'cancelled')` over the facility-tz day-range and Monday-start week-range. Uses `dayRangeInTimezone(date, tz, dayShift?)` from `src/lib/time.ts` (Phase 12T export). **Postgres driver returns iterable rows directly (no `.rows` wrapper)** — access via `(result as unknown as Array<{ n: number | string }>)[0]?.n`.
- **`src/lib/peek-drawer.ts`** — module-level handler ref. Same pattern as `tour-router.ts`: `setPeekHandler(fn)` registers the open callback; `openPeek({type, id})` calls it. Lets any component (daily log row, billing row, calendar) trigger the drawer without prop drilling.
- **`<PeekDrawer />`** (`src/components/peek-drawer/peek-drawer.tsx`) — mounted globally in `(protected)/layout.tsx` inside `<ToastProvider>` next to `<CommandPalette />`. Mounted for ALL roles (the API 403s viewers; stylists can peek their colleagues). Props: `role`, `isMaster`.
- **Mobile vs desktop** branches via `useIsMobile()` (`src/hooks/use-is-mobile.ts`). Mobile: existing `<BottomSheet>` with NO `title` prop (avatar header renders inside the scroll area). Desktop: always-mounted right-side drawer with `transform: translateX(100%) → translateX(0)` over 260ms `cubic-bezier(0.25, 0.46, 0.45, 0.94)`. Backdrop `bg-black/20 z-[79]`, panel `z-[80] w-[380px] max-w-[92vw]`, shadow `-4px 0 24px rgba(0,0,0,0.08)`. Escape closes the desktop drawer; the BottomSheet has its own escape handler. Closing the drawer waits 300ms before clearing `target`/`data` so the slide-out animation completes.
- **"View Full Profile" gating**: Resident peek hides the button when `role === 'stylist'`. Stylist peek shows it only for `role === 'admin' || role === 'bookkeeper' || isMaster`. Stylists peeking other stylists see the schedule + counts but cannot navigate to the full profile page.
- **`getInitials` was extracted** from `src/components/ui/avatar.tsx` (was internal-only) to `src/lib/get-initials.ts`. `<Avatar>` now imports from the new module — single source of truth. The peek drawer uses it directly for the 48px header avatar (Avatar doesn't have an `xl` size and the colors differ — burgundy tint for residents, gray for stylists).
- **Trigger anchors** wired in Phase 12W:
  - `data-tour="peek-resident-trigger"` on daily log resident name (`src/app/(protected)/log/log-client.tsx`) — single source for the tour engine.
  - Daily log stylist section header (`log-client.tsx`) — uses `e.stopPropagation()` because the parent row's `onClick` toggles collapsed state.
  - Billing IP view resident name (`src/app/(protected)/billing/views/ip-view.tsx`).
  - Dashboard mobile stylist booking list (`src/app/(protected)/dashboard/dashboard-client.tsx`) — guarded on `b.residentId` truthy (falls back to plain `<p>` when null).
- **Booking modal trigger SKIPPED.** The selected-resident UI is the typeahead input value, not a separate read-only display — adding a peek chip there is UX churn for a flow already covered by daily-log/calendar/billing.
- **`'PanelRight'`** added to `TutorialIcon` union (`tours.ts`) and `ICON_MAP` (`tutorial-card.tsx`). Tour `admin-peek-drawer` (4 steps): one action step targeting `[data-tour="peek-resident-trigger"]` on `/log`, three info steps. NOT added to `ONBOARDING_CHECKLIST` — peek is a discovery feature like the command palette.
- **Rate-limit bucket `peek`**: `{ tokens: 120, window: '1 m' }`. 121st request in a minute returns 429.

---

## Layout Shell (Phase 12Y → corrected in 12Y followups → iOS-fix 2026-05-17)

The protected layout (`src/app/(protected)/layout.tsx`) is a 3-zone vertical flexbox shell (4-zone with desktop sidebar). The OUTER container is `fixed inset-0 flex overflow-hidden` (NOT `h-[100dvh]` — see iOS fix note below). The INNER column is `flex-1 min-w-0 flex flex-col overflow-hidden`, containing in order: `<MobileFacilityHeader>` (shrink-0, mobile-only), `<TopBar>` (shrink-0, desktop-only), `<main className="main-content flex-1 min-h-0 overflow-y-auto overscroll-contain">`, `<MobileNav>` (shrink-0, mobile-only). **`<main>` is the ONLY vertical scroll container in the protected layout** — no nested overflow-auto.

**`position: fixed; inset: 0` (NOT `h-[100dvh]`).** `h-[100dvh]` is measured by iOS Safari BEFORE its URL-bar state is finalized on first paint — the shell ends up too tall and the mobile nav gets pushed below the visible viewport (empty gap under the nav, only fixed by rotating the device, permanent for the rest of that session). `fixed inset-0` anchors the shell to the visual viewport directly, which Safari computes correctly on first paint AND keeps in sync when the URL bar collapses/expands. This is the standard pattern for native-feeling mobile web apps. **Never revert to `h-[100dvh]`** — the bug is documented (iOS Safari ignores dynamic-viewport-units on cold load) and CSS-only patches (mixed units, fallbacks, `min-height`) don't fix it reliably.

**Why `position: fixed` on the shell is safe for `position: fixed` descendants:** per CSS spec, `position: fixed` on an element does NOT create a containing block for its `position: fixed` descendants — only `transform`, `perspective`, `filter`, `backdrop-filter`, `will-change` (of those), or `contain: layout/paint/strict/content` do. Our shell carries none of those, and neither does `html`, `body`, or `.main-content` (the Phase 12Y `transform: translateZ(0)` was removed in the 12Y followups). So `<TourModeBanner>`, `<MobileDebugButton>`, `<InstallBanner>`, `<DebugBadge>` — all `position: fixed` siblings inside the shell — still anchor to the visual viewport, exactly as they would have under `h-[100dvh]`.

**`<ProtectedBodyLock />`** (`src/components/layout/protected-body-lock.tsx`) is mounted as the first child of the shell. It flips `document.body.style.overflow = 'hidden'` and `document.documentElement.style.overflow = 'hidden'` on mount, restores the previous values on unmount. Mirrors the body-overflow idiom in `Modal` (line 26) and `BottomSheet` (line 35). Required because the shell is taken out of normal flow by `position: fixed` — without locking body/html, iOS Safari can bounce-scroll the empty document behind the shell on aggressive swipes. The lock is scoped to the protected layout — `/login`, `/family/*`, `(public)/*` (privacy, terms), `/invoice/*`, `/portal/*` are normal-flow and retain body scroll. **Never make `body { overflow: hidden }` a global rule in `globals.css`** — it would break short-screen login flows with the keyboard up.

**`html, body { height: 100% }`** in `globals.css` — kept from the 12Y followups. Defends the height chain for non-protected layouts (login, public) that still rely on `min-h-screen`. Harmless under the fixed shell since the shell ignores parent height anyway.

**`.main-content` has NO `transform`.** The Phase 12Y `transform: translateZ(0)` was REMOVED in the 12Y followups: it made `.main-content` a containing block, so `position: fixed` chrome scrolled with content instead of staying viewport-anchored (the debug button bug). `.main-content` now only carries `-webkit-overflow-scrolling: touch` (plus `overflow-y-auto overscroll-contain` Tailwind classes on the element).

**FAB / floating-chrome positioning rule (12Y followups — THE canonical rule):** Floating components are `position: fixed` and **viewport-anchored** (no containing-block trick). They clear the mobile nav using centralized CSS vars from `globals.css` `:root`:
- `--app-nav-clearance: calc(var(--app-nav-height) + var(--app-safe-bottom))` — for full-width bars that butt directly against the nav (e.g. the daily-log mobile footer bar). Desktop: `0px`.
- `--app-floating-bottom: calc(var(--app-nav-clearance) + 1rem)` — for FABs / toasts / widgets that float with a 1rem gap above the nav. Desktop: `1.5rem`.

Every floating component uses `style={{ bottom: 'var(--app-floating-bottom)' }}` (or `var(--app-nav-clearance)`). NEVER hardcode `bottom: 'calc(env(safe-area-inset-bottom) + 80px)'` and NEVER use plain `bottom-4` for nav-clearing chrome. Components currently using `--app-floating-bottom`: toast container, OnboardingChecklist, QuickBookFAB, MobileDebugButton, InstallBanner, services bulk-action bar, directory bulk-action bar. Using `--app-nav-clearance`: the daily-log mobile footer bar.

**Where floating chrome is mounted:** viewport-anchored chrome (`<MobileDebugButton>`, `<InstallBanner>`, `<TourModeBanner>`, `<DebugBadge>`) lives as a child of the shell (sibling to the inner column), OUTSIDE `<main>` — so its `fixed` positioning is viewport-relative. Note: even though the shell itself is now `position: fixed`, that does NOT make it a containing block for its `position: fixed` descendants (see the safety note above). Page-level FABs live inside their page client (also fine — there's no containing block anywhere). Don't mount viewport chrome inside `<main>` — the scroll container should not own them.

**Global overlays portal to `document.body`** via `createPortal` (`<PeekDrawer>`, `<CommandPalette>`, `<MobileTourOverlay>`). Harmless and correct; kept from Phase 12Y even though the containing-block trick is gone.

**CSS variables (`globals.css` `:root`):** `--app-header-height: 56px`, `--app-nav-height: 72px`, `--app-safe-top`, `--app-safe-bottom`, `--app-nav-clearance`, `--app-floating-bottom`. A `@media (min-width: 768px)` block overrides `--app-nav-clearance` → `0px` and `--app-floating-bottom` → `1.5rem` (no mobile nav on desktop). The old `--mobile-nav-height` / `--mobile-header-height` are DELETED.

**Body has NO `padding-top: env(safe-area-inset-top)`.** Safe-area-top is handled internally by `<MobileFacilityHeader>` (`paddingTop: 'var(--app-safe-top)'`). Safe-area-bottom is handled internally by `<MobileNav>` (`paddingBottom: 'calc(0.5rem + var(--app-safe-bottom))'`).

**`<TourModeBanner>` safe-area pattern:** the banner is `fixed top-0 left-0 right-0` with `paddingTop: 'calc(0.375rem + env(safe-area-inset-top))'` — the burgundy background fills edge-to-edge under the notch, the text sits below the safe area. It hides via `transform: translateY(-100%)` which fully clears the viewport BECAUSE the banner box starts at `top: 0` (an earlier `top: env(...)` version left the banner peeking ~19px when "hidden" — never use `top: env(...)` on a slide-up banner). Requires `viewportFit: 'cover'` in the root `layout.tsx` viewport export (already set).

---

## Tour System Platform Awareness (Phase 12Y)

The tour engine no longer fires user-facing "Couldn't find that element" toasts. When `waitForElement` returns null:
- Desktop (`tours.ts` `runStep`): `console.warn` + silent `runStep(def, index + 1)`.
- Mobile (`mobile-tour.ts` `runMobileStep`): same.

Use `console.warn` to surface drift to developers. End users never see the broken-element toast.

**`Tutorial.platform?: 'mobile' | 'desktop' | 'both'`** field gates the help catalog. `visibleFor()` in `help-client.tsx` filters by viewport via `useIsMobile()`. Undefined platform = `'both'` (default). `desktopOnly` on `TourDefinition` is REMOVED — the catalog filter is the single source of truth.

**`PLATFORM_TOUR_ALIASES`** (`tours.ts`) maps base tour ids to their platform variants:
```ts
...
```
At `startTour(tourId)` call time, if `tourId` is a base id with an alias, it resolves to the right variant via `isMobile()`. This lets `ONBOARDING_CHECKLIST`, the `<HelpTip tourId="stylist-calendar">` injection points, and external `?tour=stylist-calendar` query params continue to work without per-call-site changes.

**`isTourCompleted(baseId, completedTours)`** helper exported from `tours.ts` — checks if either the base id or any of its platform variants appear in the user's `completed_tours` array. Used by `<OnboardingChecklist>` so completing `stylist-calendar-mobile` checks off the `stylist-calendar` checklist row.

**Stylist tours split into mobile + desktop variants:**
- `stylist-getting-started-mobile` / `-desktop` (the calendar overview step diverges — mobile targets `[data-tour="stylist-mobile-booking-list"]`, desktop targets `[data-tour="calendar-time-grid"]`).
- `stylist-calendar-mobile` / `-desktop` (FullCalendar grid on desktop vs today's-bookings list on mobile).

**Other stylist tours stay as `platform: 'both'`**: `stylist-daily-log`, `stylist-checkin`, `stylist-residents`, `stylist-finalize-day`, `stylist-my-account`, `stylist-signup-sheet`. They render the same UI on both viewports.

**`platform: 'desktop'` tours (12Y followups audit)** — tours covering routes NOT reachable from the mobile nav (`/billing`, `/stylists`, `/stylists/directory`, `/master-admin/*`, plus data-heavy `/analytics`/`/payroll`): `admin-command-palette`, `admin-reports`, `admin-compliance`, `bookkeeper-getting-started`, `bookkeeper-billing-dashboard`, `bookkeeper-payroll`, `master-getting-started`, `master-add-facility`, `master-stylist-directory`, `master-applicant-pipeline`, `master-quickbooks-setup`, `master-analytics`. **`bookkeeper-scan-logs` / `bookkeeper-manual-entry` / `bookkeeper-duplicates` stay `'both'`** — they target `/log` and `/residents` (mobile-reachable; OCR scanning works on a phone).

**New `data-tour` anchors** in `dashboard-client.tsx` stylist mobile view: `stylist-mobile-booking-list` on the booking-cards wrapper, `stylist-mobile-booking-card` on the first card (or empty-state card).

**`scripts/check-tours.ts`** extended with a platform-consistency pass: for each tour tagged `mobile` or `desktop`, scan the anchor element's source-file context for `md:hidden` (mobile-only) or `hidden md:flex|block|grid|inline` (desktop-only) class patterns. Emits warnings on mismatch; does not fail the script (heuristic, false positives possible).

**Tour speed (12Y followups):** `SLOW_PAGE_WAIT_MS` is `3000` (was 5000) in both `tours.ts` and `mobile-tour.ts` — the missing-element timeout. `SCROLL_SETTLE_MS` is `25` (was 50) in `mobile-tour.ts`. `waitForElement` is MutationObserver-based and resolves instantly when the element appears; these constants only bound the worst case. Don't add artificial step-to-step delays — the only two `setTimeout(…, 50)` calls (action-step click handling) are necessary for React to process the click.
...
- **`is_demo` filter (Phase 13-Tutorial)** — any new user-facing list query on `residents`, `stylists`, `services`, `bookings`, `log_entries`, `qb_invoices`, `qb_payments`, `pay_periods`, or `stylist_pay_items` MUST add `eq(table.isDemo, false)` with comment `// is_demo filter — Phase 13`. **During an active scripted tour the filter flips to `eq(table.isDemo, tutorialMode)`** (demo-only in a tour, real-only otherwise) via `isTutorialModeActive()` / `isTutorialRequest(request)`. Do NOT add the filter to internal queries (auth lookups, by-ID mutations, role checks) — only at user-facing list-rendering sites. `logEntries`, `stylistCheckins`, `signupSheetEntries`, `qbInvoices`, `qbPayments`, `payPeriods`, `stylistPayItems` have no `active` column — demo records are hard-deleted by the reset endpoint (`DELETE /api/help/demo-data`) and the weekly cleanup cron (deleting a demo `pay_period` cascades its pay items + deductions).
- **Billing/payroll demo data (Phase 13-Tutorial, 2026-06-02)** — `is_demo` added to `qb_invoices`, `qb_payments`, `pay_periods`, `stylist_pay_items` (`drizzle/0004_billing_payroll_is_demo.sql`). `seedFacilityDemoData` seeds a demo open invoice + received check for Mrs. Smith and a demo monthly pay period (one item for Demo Sarah) and returns the period id as scenarioState slug `demo-pay-period` (used by `scripted-bookkeeper-payroll`'s `/payroll/{{demo-pay-period}}` route templating). The billing-summary `unstable_cache` takes `demo` as its 4th arg so demo/real snapshots cache separately. `/api/reports/monthly` + `/api/reports/invoice` gained the `is_demo` filter — this ALSO fixed a latent leak where the seeded demo booking was aggregated into REAL analytics revenue (dashboard + super-admin reports already filtered correctly).
- **Scripted tours launched from `/help` (`TutorialCard`) MUST `router.refresh()` after `seedAndStart`/`startScriptedTour`** — otherwise the destination page's server components are served from the stale pre-tour Router Cache snapshot (no tutorial cookie) and SSR-fed demo lists (walk-in resident search, daily-log rows, billing/payroll) appear empty. `advanceStep()` also refreshes after a cross-route `router.push`. Mirrors `FirstTourAutoLauncher`.
- **Scripted-tour click advance — `click` vs `pointerdown`**: `wireAdvanceOnClick(selector, index, usePointerDown=false)` in `scripted-tour.ts`. Regular `type:'click'` steps (FAB, nav, submit buttons) advance on **bubble-phase `click`** so React's own `onClick` (open modal, navigate) runs first; advancing on capture-phase `pointerdown` fired ~100-300ms too early on mobile and broke the FAB→modal open. Typeahead `advanceSelector` options use **capture-phase `pointerdown`** because they call `onMouseDown` and remove themselves from the DOM before `click` fires.
...
- **Phase 12O** (SHIPPED 2026-05-11) — Tour Mode Write Interception: `window.fetch` patched while tour active; `/api/*` writes return fake 200s (`buildFakeResponse` smart-shape table); `GET`/`HEAD` and external URLs pass through. New modules `tour-mode.ts`, `tour-fetch-interceptor.ts`, `tour-mode-banner.tsx`. Burgundy banner pinned `top: 0 z-[250]` while active. `setTourModeActive(false)` only at the two truly terminal exits per engine (NOT inside `destroyActiveTour()`). `tour-resumer.tsx` re-installs the patch on mount after a cross-route reload. 6 write-action `actionHint` strings rewritten to reassure users.
- **Phase 12P** (SHIPPED 2026-05-11) — Client-Side Tour Navigation: `router.push()` SPA nav replaces `window.location.href` hard-reload for cross-route tour steps. Module-level router ref via `tour-router.ts` + `<TourRouterProvider />`. `waitForElement` rewritten as MutationObserver (instant resolve when element in DOM). Fallback to hard-nav + sessionStorage resume preserved when router ref is null.
- **Phase 12Q** (SHIPPED 2026-05-11) — Tour Completion Tracking: `profiles.completed_tours text[]` column, `POST /api/profile/complete-tour` (idempotent `array_append`, rate-limited), `✓ Done` badge on Help page tutorial cards, optimistic update via `tour-completed` CustomEvent.
...
- **Phase 12T** (SHIPPED 2026-05-12) — "I'm Here" Stylist Check-In + Smart Day Rescheduling: new `stylist_checkins` table (`stylistId`, `facilityId`, `date`, `checkedInAt`, `delayMinutes`, unique `(stylistId, facilityId, date)`, RLS `service_role_all`); `POST /api/checkin` (stylist-only, idempotent via unique constraint, `checkin` rate-limit 10/h/user); `PUT /api/bookings/bulk-reschedule` (stylist-only, single transaction, `bulkReschedule` rate-limit 20/h/user); `<CheckInBanner>` + `<RescheduleSheet>` components under `src/components/checkin/`; dashboard wires two stylist-only queries (today's checkin row + today's bookings) inside the existing `Promise.all` and passes `alreadyCheckedIn` + `checkinTodayBookings` props (NOT `todayBookings` — that name collides with an existing local in `dashboard-client.tsx`); banner mounts in both mobile-stylist and desktop calendar paths; `stylist-checkin` tour added to TUTORIAL_CATALOG + TOUR_DEFINITIONS + ONBOARDING_CHECKLIST.stylist (as item #4); `Clock` lucide icon added to `TutorialIcon` union + `ICON_MAP`; `dayRangeInTimezone` extracted from `src/lib/reconciliation.ts` to `src/lib/time.ts` as a public export. **Internal only — no GCal sync** on bulk-reschedule (out of scope; documented).

- **Phase 12U** (SHIPPED 2026-05-12) — Overscroll Lock + Native Touch Polish: `html, body { overscroll-behavior: none }` (iOS rubber-band bounce), `* { -webkit-tap-highlight-color: transparent }` (tap flash), `button/[role="button"]/a/label { user-select: none; -webkit-user-select: none }` (accidental text selection), `.main-content { -webkit-overflow-scrolling: touch }` (momentum scroll). CSS-only — no component, route, or schema changes.
- **Phase 12V** (SHIPPED 2026-05-12) — CMD+K Command Palette (desktop only). `GET /api/search?q=…` (admin/bookkeeper/master only, `search` rate-limit 60/min/user, residents inner-joined to facilities, stylists DISTINCT-joined to assignments for non-master, LEFT-joined to facilities for master). New `<CommandPalette />` mounted in `(protected)/layout.tsx` inside ToastProvider, gated on role. Sidebar search trigger (`data-tour="cmd-k-trigger"`, `hidden md:flex`) dispatches `'open-command-palette'` CustomEvent. New static `PALETTE_ROUTES` table in `src/lib/command-palette-pages.ts`. Tour `admin-command-palette` (4 steps, desktopOnly) added. `Search` lucide icon added to `TutorialIcon` union + `ICON_MAP`.
- **Phase 12W** (SHIPPED 2026-05-12) — Resident/Stylist Peek Drawer. `GET /api/peek?type=resident|stylist&id=<uuid>` (all roles except viewer, `peek` rate-limit 120/min/user, cross-facility peeks return 404 to avoid existence leak). New module-level state `src/lib/peek-drawer.ts` (mirrors `tour-router.ts`) with `setPeekHandler` + `openPeek`. `<PeekDrawer />` mounted globally in `(protected)/layout.tsx` for all roles — `useIsMobile()` branches between desktop right-drawer (`translateX` 260ms) and the existing `<BottomSheet>` on mobile. `getInitials` extracted from `avatar.tsx` to `src/lib/get-initials.ts`. Trigger anchors wired: daily log resident name (with `data-tour="peek-resident-trigger"`), daily log stylist header, billing IP view resident name, dashboard mobile stylist booking list. Booking modal SKIPPED (typeahead input — no read-only display). Tour `admin-peek-drawer` (4 steps) added; `PanelRight` lucide icon added to `TutorialIcon` union + `ICON_MAP`. NOT in `ONBOARDING_CHECKLIST` — discovery feature like 12V.
- **Phase 12X** (SHIPPED 2026-05-12) — Fluid Typography + Skeleton/page-enter Audit. CSS-only changes plus 4 className additions and 1 new `loading.tsx`. Global `h1 { font-size: clamp(1.625rem, 4vw, 2.25rem) }` in `globals.css` scales DM Serif Display headings from 26px (320px phone) → 36px (large monitor); a more aggressive `.dashboard-greeting { font-size: clamp(1.75rem, 5vw, 2.5rem) }` is applied to the homepage greeting (alongside its existing `text-2xl` Tailwind class — source-order cascade lets the custom rule win). Skeleton audit: `stylists/directory/loading.tsx` was the only missing route — created with header + search bar + 8 row skeletons using `.skeleton rounded-2xl`. Audit confirmed all other routes flagged by prior CLAUDE.md inventory are present on disk. `page-enter` mount animation added to four newer page clients that were missed during the design-system rollout: `master-admin-client.tsx` (line 408), `services-page-client.tsx` (line 201), `analytics/reports-client.tsx` (line 185), `residents/import/import-client.tsx` (line 217). Dashboard explicitly excluded per existing rule. `onboarding-client.tsx` excluded — full-height centered wizard layout would fight the translate-Y entrance.
- **Phase 12Y** (SHIPPED 2026-05-13) — Mobile Layout Shell + Tour System Overhaul + Directory Scroll Fix. Three-part polish landing together. **Shell:** `(protected)/layout.tsx` restructured to 3-zone flexbox shell — outer `h-[100dvh] flex overflow-hidden`, inner column with `<MobileFacilityHeader>` / `<main>` / `<MobileNav>` as flex siblings. MobileNav no longer `position: fixed`. `.main-content` carries `transform: translateZ(0)` (replaces `will-change: scroll-position`) — creates a containing block for `position: fixed` descendants AND a compositor layer for scroll. Eight floating components dropped their inline `calc(env(safe-area-inset-bottom) + 80px)` offsets and use plain Tailwind `bottom-4` (toast, OnboardingChecklist, QuickBookFAB, log/services/directory FABs, MobileDebugButton, InstallBanner). MobileDebugButton + InstallBanner moved INSIDE `<main>` so they benefit from the containing block. `<CommandPalette>`, `<PeekDrawer>`, `<MobileTourOverlay>` portaled via `createPortal(document.body)` to remain viewport-spanning. Renamed CSS vars: `--mobile-nav-height` → `--app-nav-height`, `--mobile-header-height` → `--app-header-height`; added `--app-safe-top` / `--app-safe-bottom`. Removed `body { padding-top: env(safe-area-inset-top) }` — header handles it internally. **Tour engine:** `waitForElement` returning null now silently `console.warn` + advances; no more user-facing "Couldn't find that element" toast. **Type changes:** `Tutorial.platform?: 'mobile' | 'desktop' | 'both'` field added; `TourDefinition.desktopOnly` REMOVED (catalog filter is single source of truth). `help-client.tsx::visibleFor()` extended with `useIsMobile()` viewport filter. `PLATFORM_TOUR_ALIASES` map resolves base tour ids to platform variants in `startTour()`; `isTourCompleted(baseId, completed[])` helper checks completion across variants for ONBOARDING_CHECKLIST. **Stylist tour splits:** `stylist-getting-started` + `stylist-calendar` each split into `-mobile` and `-desktop` variants targeting platform-specific anchors (`[data-tour="stylist-mobile-booking-list"]` + `[data-tour="stylist-mobile-booking-card"]` on mobile; `[data-tour="calendar-time-grid"]` + `[data-tour="calendar-today-btn"]` on desktop). `admin-command-palette` migrated to `platform: 'desktop'`. **Directory scroll:** root-cause was the `will-change: scroll-position` on `.main-content` blocking wheel-event delegation on Safari/macOS trackpad; replaced with `transform: translateZ(0)` (same compositor promotion, no event blocking). **`scripts/check-tours.ts`** extended with a platform-consistency pass that scans anchor-element source context for `md:hidden` (mobile-only) / `hidden md:flex` (desktop-only) class patterns and emits warnings on platform-tour mismatches (heuristic, doesn't fail the script).
- **Phase 12Y followups** (SHIPPED 2026-05-13) — Fixed 7 regressions from the 12Y shell. **Bottom nav gap:** added `html, body { height: 100% }` so `h-[100dvh]` shell fills the viewport. **Containing-block trick REVERSED:** `transform: translateZ(0)` removed from `.main-content` — it made `position: fixed` chrome scroll with content (debug button bug). Replaced with centralized `--app-nav-clearance` + `--app-floating-bottom` CSS vars (with `md:` overrides); all 8 floating components switched from plain `bottom-4` to `style={{ bottom: 'var(--app-floating-bottom)' }}` (daily-log footer bar uses `--app-nav-clearance`). MobileDebugButton + InstallBanner moved OUT of `<main>` to outer-shell siblings. **Tour banner:** `top: env(...)` → `top: 0` + `paddingTop: calc(0.375rem + env(safe-area-inset-top))` — fixes both the notch cutoff AND the "banner peeks ~19px when hidden" bug (`translateY(-100%)` now fully clears it). **Directory scroll:** removing `transform: translateZ(0)` (it was the leading cause + a failed prior fix) plus moving the `selectAllRef.current.indeterminate` write out of the render body into a `useEffect`. **Tour audit:** 11 catalog entries tagged `platform: 'desktop'` (admin-reports, admin-compliance, 3 bookkeeper, 6 master) — they cover routes not in the mobile nav. bookkeeper scan-logs/manual-entry/duplicates stay `'both'`. **Tour speed:** `SLOW_PAGE_WAIT_MS` 5000 → 3000 (both engines), `SCROLL_SETTLE_MS` 50 → 25 (mobile).
- **Email Day Log** (SHIPPED 2026-06-11) — `POST /api/log/email` lets any facility role (except viewer) email a formatted HTML day log to any address (rollout helper — typically the facility office). Body `{ date, to (email ≤320), message? (≤500) }`, Zod-validated, rate-limited `logEmail` 10/h/user. Stylist role auto-scoped to own bookings via `profiles.stylistId` (same pattern as `/log` page). Query: non-cancelled, `active=true`, `is_demo=false` (ALWAYS false — never email demo data, even mid-tutorial; the tour fetch interceptor fakes the POST client-side anyway) bookings in the UTC day window. **`buildDailyLogEmailHtml`** in `src/lib/email.ts` follows the house email style (burgundy `#8B2E4A` header, 560px card, summary tiles Appointments / Total / Tips, per-stylist sections when >1 stylist, status + payment badges per row, italic notes, `EMAIL_FOOTER`). Tips tile only renders when tips > 0; revenue total is `priceCents + addonTotalCents` only (never tip_cents — guard comment present). All user strings pass through the new `escHtml()` helper. Send is fire-and-forget per the email convention. **UI**: Email button on `/log` header between the date arrows and Export (`data-tour="log-email-day"` + `data-tour-mobile` — anchor reserved, no tour step yet), opens `<EmailDayLogModal>` (`src/components/exports/email-day-log-modal.tsx`) — desktop `<Modal>` / mobile `<BottomSheet>` via `useIsMobile()`, recipient email + optional ≤500-char message, success/error toasts, button disabled until email regex passes.

- **Phase 12Z** (SHIPPED 2026-05-17) — Daily Log Excel Export (bookkeeper format). New dep `exceljs` (`xlsx` doesn't support styled writes in the open-source build — kept for imports only). New `GET /api/exports/daily-logs?facilityIds=…&startDate=…&endDate=…` route emits a styled `.xlsx` matching the `F177 - Sunrise of Bethesda.xlsx` reference sheet: 14 columns (`No., Mail Subject, Mail date, Mail Time, Service Date, Facility Name, Stylist Name, Client Name, Room#, Services Performed, Amount, Notes, Tips, Payment Type`), yellow header fill (`FFFFFF00` solid), Calibri font, bold on every header EXCEPT B/C/D (`Mail Subject`/`Mail date`/`Mail Time`) — that quirk is intentional and matches the sample. Column widths from spec: C=11.33, D=9.66, E=12.89, F=26.66, G=20.55, H=22.44, N=14.22. Filters: `bookings.status = 'completed'` AND `bookings.active = true` AND date range (per-facility tz via `dayRangeInTimezone` with widest-UTC over-fetch + per-row tz re-filter). No `log_entries.finalized` filter — every completed booking exports. Stylist role is auto-scoped to own bookings via `profiles.stylistId`. Master admin bypasses facility scope. `exportExcel` rate-limit bucket: 10/min/user keyed on `u:${user.id}`. **Payment Type mapping** (col N): `unpaid → "Invoice"`, `paid → "Other"`, `waived → "Other"`, null → `"Doesn't Fill"`. NO `qb_payments.payment_method` join — accepted tradeoff to keep the query simple; if bookkeepers want Cash/Check distinction, revisit. **"Doesn't Fill" placeholder** (NOT empty string, NOT null) is used for: empty room, empty notes, null/zero tips, unknown payment type, missing services. Shared formatters live in `src/lib/exports/daily-log-format.ts` (`formatServices`, `paymentTypeLabel`, `dollarsNumber`, `tipsCell`, `facilityLabel`, `stylistLabel`, `notesCell`, `roomCell`, `NOT_FILLED`). Two modals in `src/components/exports/`: `ExportDailyLogsModal` (single-facility, used on `/log`) and `ExportDailyLogsMultiModal` (checkbox list of accessible facilities + date range, used on `/analytics`). Both branch desktop `<Modal>` vs mobile `<BottomSheet>` via `useIsMobile()`. `/log` button is in the header next to the date arrows with `data-tour="log-export-excel"` + `data-tour-mobile="log-export-excel"` (icon-only on mobile, icon+"Export" label on md+). `/analytics` button is in the header next to the month picker with `data-tour="analytics-export-excel"`. `analytics/page.tsx` fetches accessible facilities server-side (master → all active; others → `facilityUsers` memberships) and passes as `exportFacilities` prop. New tour `bookkeeper-export-logs` (3 steps, `platform: 'both'`, `roles: ['bookkeeper', 'admin', 'super_admin']`, icon `FileSpreadsheet` — already in `ICON_MAP`). NOT added to `ONBOARDING_CHECKLIST` — discovery feature like 12V/12W. **Service concatenation:** uses denormalized `bookings.serviceNames[]` (Phase 12B), falls back to `bookings.service.name`. Amount = `(priceCents ?? 0) + (addonTotalCents ?? 0)` / 100. **Stylist code: NOT a new column.** `stylists.stylist_code TEXT NOT NULL UNIQUE` already existed (format ST### per `/^ST\d{3,}$/` regex in `POST /api/stylists`) — used directly, no migration.

- **Phase 13-Tutorial** (SHIPPED 2026-05-29) — Scripted Tutorials with Demo Characters. New `is_demo boolean default false NOT NULL` column on `residents`, `stylists`, `services`, `bookings`, `log_entries`, `stylist_checkins`. Partial index `residents_demo_idx (facility_id) WHERE is_demo = true`. New `profiles` columns: `has_seen_first_tour boolean default false NOT NULL`, `help_progress jsonb` (nullable, type `{ tourId, stepIndex, scenarioState } | null`). New `help_step_events` table (id, facility_id nullable FK, user_id nullable FK, tour_id, step_index, action text, created_at; indexes `(tour_id, step_index, action)` and `(facility_id, created_at)`). New demo seeder `src/lib/help/demo-seeder.ts` (`seedFacilityDemoData`, `getDemoIds`) — idempotent, seeds Mrs. Margaret Smith (Rm 12), Mr. Robert Johnson (Rm 8), Wash & Set $35, Haircut $25, Demo Sarah stylist with Mon–Fri availability. New scripted tour engine `src/lib/help/scripted-tour.ts` coexisting with the legacy Driver.js engine — 10 tours defined in `src/lib/help/tours-stylist-mobile.ts` and `src/lib/help/tours-stylist-desktop.ts`. New visual overlay components under `src/components/help/scripted-tour/`: `spotlight-mask.tsx` (SVG mask full-viewport cutout), `spotlight-ring.tsx` (pulsing `position: fixed` ring), `target-arrow.tsx` (quadratic bezier SVG arrow with `nearestEdgeMidpoint()`), `scripted-tour-popover.tsx` (320px desktop card, auto-positioning, keyboard nav), `scripted-tour-sheet.tsx` (mobile bottom sheet with swipe), `tutorial-celebration.tsx` (CSS confetti + learnings list + CTAs). `<ScriptedTourOverlay>` mounted in `(protected)/layout.tsx` alongside `<PeekDrawer>`. `<FirstTourAutoLauncher>` in `DashboardClient` fires for new users (`hasSeenFirstTour = false`) after 1.5s. Help page upgraded with search bar and resume banner reading `sessionStorage`/`helpProgress`. Settings → Advanced gains "Tutorial Data" card to wipe demo data (`DELETE /api/help/demo-data`). New rate-limit buckets: `helpSeed` (5/hr/user), `helpTrack` (60/min/user). New cron `GET /api/cron/help-demo-cleanup` (weekly Sunday 03:00 UTC) hard-deletes `is_demo=true AND active=false` records older than 90 days. **Run `npx dotenv -e .env.local -- npx drizzle-kit push` to apply schema changes.**

- **Phase 13-Tutorial-Interactive** (SHIPPED 2026-05-29) — Made the 5 stylist scripted tours genuinely INTERACTIVE (user clicks real buttons, text auto-fills) and creating REAL throwaway demo records instead of faking writes. Schema applied via committed idempotent SQL `drizzle/0001_phase13_demo_tutorial.sql` (drizzle-kit push aborts on its interactive prompt — apply SQL files directly with `psql "$DIRECT_URL" -f ...`).
  - **Tutorial-mode signal is a COOKIE, not a header.** `ss_tutorial_mode=1` (15-min TTL) set by `tutorial-cookie.ts::setTutorialCookie()` on tour start, cleared on close/complete. Cookies ride every request automatically so BOTH server components (SSR page queries feeding props like the booking modal's resident/service lists) and API route handlers read it. Server readers: `tutorial-request.ts` — `isTutorialRequest(request)` (route handlers, parses the cookie header) and `isTutorialModeActive()` (server components, via `next/headers` cookies). **Do NOT switch to a header** — SSR can't read a client-set header.
  - **Reads are DEMO-ONLY during a tour** (`eq(table.isDemo, tutorialMode)`), NOT demo+real. This makes the tour a clean sandbox and prevents accidentally mutating a real booking during the mark-paid step. Applied at: GET `/api/bookings`,`/api/log`,`/api/residents`,`/api/services`,`/api/stylists`; SSR `dashboard/page.tsx` + `log/page.tsx`; and `resolveAvailableStylists({ demoOnly })`. **`/api/services` + `/api/stylists` GET previously had NO is_demo filter — added (would have leaked demo records into real lists once seeded).**
  - **Writes tag `is_demo`** when `isTutorialRequest(request)`: POST `/api/bookings` (+ skips GCal sync + confirmation email when `isDemo`), `/api/log`, `/api/residents`, `/api/services`, `/api/stylists`.
  - **Demo booking is assigned to the VIEWER's `stylistId`** (seed route looks up `profiles.stylistId`), NOT Demo Sarah — because the daily log + dashboard filter to `profile.stylistId` (`stylistFilter`), so a Sarah-owned booking would be invisible to the stylist running the tour. `seedFacilityDemoData(facilityId, viewerStylistId?)`. The seeder also seeds a TODAY booking for Mrs. Smith so daily-log/check-in tours have a row.
  - **Engine (`scripted-tour.ts`)**: `startScriptedTour` sets the cookie + navigates to step 0's `route`. `wireStep` per step: `type:'click'` → `waitForElement` then a one-time capture-phase listener on **`pointerdown` AND `click`** (pointerdown is essential for targets that vanish on mousedown, e.g. the resident typeahead option that closes its own dropdown) → 50ms settle → `advanceStep`. `type:'type'` → `waitForElement` then `autoFillInput` (native setter, handles `<input>/<textarea>/<select>`). `typeValue` supports `{{slug}}` placeholders resolved against seeded `scenarioState` IDs, plus the special `{{tomorrow-10am}}` token. `seedAndStart(tourId)` seeds demo data + passes IDs as scenarioState; the launcher + `TutorialCard` call it and `router.refresh()` so SSR re-renders with demo props. The terminal step auto-shows the celebration (no explicit `celebrate` step).
  - **Action vs info visuals**: `spotlight-mask.tsx` is now FOUR dark panels around the cutout (NOT an SVG `<mask>`) so real clicks reach the target; panel-click dismiss is suppressed on action steps. `spotlight-ring` + `target-arrow` take `isAction` (`step.type==='click'`): action steps get a bright white highlight + glow pulse and a bigger (18px) solid full-opacity arrow; info steps stay subtle. Popover/sheet hide Next on action steps and show a "👆 click the highlighted spot" hint; keyboard/swipe forward-advance disabled on action steps.
  - **`TutorialCard`** routes the 5 stylist tours (`stylist-getting-started/calendar/daily-log/checkin/finalize-day`) to their `scripted-stylist-*-mobile/-desktop` variants via `SCRIPTED_TOUR_MAP` + `seedAndStart`; other tours keep the legacy Driver.js engine.
  - **`scripts/check-tours.ts`** now also validates every `selector:` in `tours-stylist-{mobile,desktop}.ts` against `data-tour`/`-mobile` anchors (exits 1 on miss). New anchors: `booking-modal-resident`, `booking-modal-resident-option`, `booking-modal-service`, `booking-modal-date`, `booking-modal-submit`; `dashboard-new-booking-fab` (mobile FAB); `log-payment-toggle`. Desktop new-booking uses the existing `calendar-time-grid` (dateClick opens the modal).

- **Phase 13-Tutorial-Interactive Batch 3+4** (SHIPPED 2026-05-29) — Extended interactive scripted tours to the remaining create/mutate flows. New tour files `tours-facility-staff.ts` (residents add, scheduling mobile+desktop, sign-up sheet), `tours-admin.ts` (residents add), `tours-bookkeeper.ts` (manual walk-in entry). All registered in `scripted-tour.ts`'s `allTours`, mapped in `SCRIPTED_TOUR_MAP` (`tutorial-card.tsx`), and validated by `scripts/check-tours.ts` (`SCRIPTED_FILES` now lists all 6 tour files).
  - **SEEDED vs UNSEEDED**: residents-add tours (`scripted-facility-staff-residents`, `scripted-admin-residents`) are in `UNSEEDED_SCRIPTED_TOURS` (they create their own demo record via the form). Scheduling + bookkeeper-walk-in + signup-sheet tours are SEEDED (`seedAndStart`) because their forms need an existing demo resident (Mrs. Smith) + service (Wash & Set) to fill.
  - **Booking modal default service row**: `booking-modal.tsx` now initializes `selectedServiceIds` to `['']` (was `[]`) in create mode so the service `<select>` (`data-tour="booking-modal-service"`) renders immediately — both the stylist-calendar tour AND facility-staff-scheduling depend on it. Empty ids are filtered before submit (`submitDisabled` requires `.filter(id=>!!id).length>0`), so this is a no-op for real users beyond showing the dropdown sooner.
  - **Bookkeeper `POST /api/bookings`**: the role guard now allows `bookkeeper` (manual walk-in entry is the non-OCR equivalent of log-sheet entry they already do). The `/log` walk-in form already showed for bookkeeper via `canWrite`; the API was the only blocker.
  - **`signup_sheet_entries.is_demo`** column added (`boolean NOT NULL DEFAULT false`) — schema.ts + idempotent `drizzle/0003_signup_sheet_is_demo.sql`. `GET /api/signup-sheet` now filters `eq(isDemo, isTutorialRequest(request))` (previously had NO demo filter — a latent leak: demo entries would have shown in real lists). `POST` tags `isDemo`. Demo signup entries are hard-deleted by `DELETE /api/help/demo-data` and the weekly `help-demo-cleanup` cron (no `active` column).
  - **`{{tomorrow}}` token** added to `resolveTypeValue` (date-only `YYYY-MM-DD`, alongside `{{tomorrow-10am}}`) for the sign-up-sheet preferred-date `type` step.
  - **New `data-tour` anchors**: `residents-add-name` + `residents-add-submit` (residents-page-client); `walkin-resident-search`, `walkin-resident-option`, `walkin-service-select`, `walkin-submit` (log-client walk-in form); `signup-sheet-resident`, `signup-sheet-resident-option`, `signup-sheet-service`, `signup-sheet-service-option` (signup-sheet-panel).
...
- **Phase 13-Tutorial-Mobile** (SHIPPED 2026-05-31) — Mobile tutorial coverage for every role. **Root cause it fixes:** master admin saw an EMPTY help catalog on mobile because all 9 master cards were `platform: 'desktop'` and `visibleFor()` (in `help-client.tsx`) hides wrong-platform cards. On mobile, master admin uses the standard **admin bottom nav** (Calendar, Log, Residents, Analytics, Payroll, Settings) — the master-only screens (`/master-admin/*`, Stylist Directory, Billing) have NO mobile nav entry.
  - **7 new master mobile cards** in `TUTORIAL_CATALOG` (`masterOnly: true, platform: 'mobile'`): `master-getting-started-mobile`, `master-calendar-mobile`, `master-daily-log-mobile`, `master-residents-mobile`, `master-analytics-mobile`, `master-payroll-mobile`, `master-settings-mobile`. Backed by 7 new `platform: 'mobile'` scripted tours appended to `tours-master.ts` (seeded highlight/info style — anchored to the mobile bottom nav via `[data-tour="nav-*"]` + page anchors). Each ends with a "deeper tools / facilities / franchises / billing live on the full website" pointer step (empty-selector info).
  - **Mobile gap re-tags** (`desktop` → `both`): `admin-reports`, `bookkeeper-payroll`, `bookkeeper-financial-reports`, `bookkeeper-getting-started`. Analytics + Payroll ARE in the mobile nav and their anchors (`analytics-*`, `payroll-*`) render on mobile (verified: no `hidden md:` gating), so the existing scripted tours work on a phone. Billing/QuickBooks stay `desktop` (no mobile nav).
  - **`scripted-bookkeeper-getting-started-mobile`** added to `tours-bookkeeper.ts` — the desktop getting-started highlights `billing-outstanding` (billing has no mobile nav), so the mobile variant covers Log/Analytics/Payroll + a "Billing & QuickBooks on desktop" pointer. `SCRIPTED_TOUR_MAP['bookkeeper-getting-started'].mobile` points to it; `.desktop` keeps the original.
  - **Mobile-tour authoring rule**: anchor mobile tours to `[data-tour="nav-calendar|nav-daily-log|nav-residents|nav-analytics|nav-payroll|nav-settings"]` — `resolveQuery()` maps these to the mobile nav's `data-tour-mobile` slugs (set via the `tourSlug` ternary in `mobile-nav.tsx`), and `check-tours.ts` `found()` matches the literal slug string in that file. For features with no mobile nav (facility setup deep config, franchises, merge, billing, directory), use an empty-selector (`''`) info step that points the user to the desktop site rather than anchoring a missing element.
  - `SCRIPTED_TOUR_MAP` now 46 entries. check-tours 292/292. TSC clean. No schema changes.

- **Phase 13-Tutorial-Mobile followups** (SHIPPED 2026-05-31) — scripted-tour overlay UX fixes (all in `src/components/help/scripted-tour/`):
  - **The dark panels NEVER close the tour.** `spotlight-mask.tsx` dropped its `onClose`/`panelClose` — tapping outside the spotlight (or tapping the highlighted element on an action step) no longer exits. Only the explicit **X** button (sheet/popover) — and **Esc** on desktop — closes a tour. This was a top complaint: users clicked the thing the tour pointed at (e.g. a Back/home link) and the tour died. `SpotlightMask` no longer takes `isAction` either (was only used for the cursor).
  - **Mobile sheet floats above the bottom nav.** `scripted-tour-sheet.tsx` changed `bottom: 0` → `bottom: var(--app-nav-clearance, 0px)` and padding → flat `20px`. Before this, the FIRST step's Next/"Get Started" button was hidden behind the mobile nav (later steps have a Back button below Next, so Next happened to clear the nav — that's why only step 0 looked broken). Floating above the nav also keeps a highlighted nav item visible below the sheet.
  - **Action-step targets are elevated above the sheet.** `scripted-tour-overlay.tsx` adds an effect: for `type:'click'` steps it sets the target's `z-index: 9015` (and `position: relative` if static), restored on cleanup. Needed because bottom-anchored targets like the mobile **`QuickBookFAB`** (`z-40`, `--app-floating-bottom`) sit *behind* the bottom sheet (`z-9010`) and were untappable. The booking modal (`z-50`, top-aligned `pt-16`) is unaffected — its inputs are above the sheet already.
  - **`data-tour-mobile="mobile-home-logo"`** added to the `MobileFacilityHeader` logo `<Link>` so a tour can point an arrow at the top-left home button.
  - **Master mobile tours made interactive** (`tours-master.ts`): `scripted-master-getting-started-mobile` is now a tap-through (action clicks on `nav-daily-log`/`nav-residents`/`nav-analytics`/`nav-payroll` + a `mobile-home-logo` arrow step), `scripted-master-calendar-mobile` is a real FAB→booking flow (mirrors the stylist booking tour, seeded), `scripted-master-residents-mobile` is a real +→type name→submit add (seeded). Data-reading tours (daily-log/analytics/payroll/settings) stay highlight/info. check-tours 297/297.

- **Phase 13-Tutorial-Getting-Started-Interactive** (SHIPPED 2026-05-31) — every role's `getting-started` orientation was passive (highlight-only); rewrote them all into interactive **"walk your whole day, start to finish"** tours that click through the user's real nav and point out the key action on each landed page. **The desktop sidebar exposes the SAME `data-tour="nav-*"` anchors as the mobile bottom nav** (both via the `tourSlug` ternary in `sidebar.tsx` + `mobile-nav.tsx`), so `[data-tour="nav-calendar|nav-daily-log|nav-residents|nav-analytics|nav-payroll|nav-settings|nav-billing|nav-master-admin]` CLICK steps work on BOTH platforms via `resolveQuery` (desktop matches `data-tour`, mobile matches `data-tour-mobile`). Tours rewritten: stylist (mobile + desktop variants), `scripted-staff-getting-started`, `scripted-admin-getting-started`, `scripted-bookkeeper-getting-started` (desktop) + `scripted-bookkeeper-getting-started-mobile`, `scripted-master-getting-started` (desktop; the `-mobile` was already interactive). The scripted `platform` field stays `'mobile' | 'desktop'` (no `'both'` — the catalog card's `platform` controls visibility); staff/admin getting-started cards are `platform: 'both'` and run the single desktop-authored tour on both, with nav clicks that resolve per-device. check-tours 312/312, tsc clean. No schema changes.
