...
- **Services page list:** `services-page-client.tsx` renders rows interleaved with category section headers (sorted alphabetical, "Other" last — the sort key uses `|| 'zzzOther'` to force that). Pattern is an IIFE that iterates a sorted array and pushes `<div>{category}</div>` + service rows into a `nodes: ReactNode[]` array, returned as the JSX children. Do NOT revert to a flat `services.map()`. Category sub-label under each service name is kept even with section headers — it remains useful when the list is filtered.
