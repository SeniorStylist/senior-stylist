...
- **`is_demo` filter (Phase 13-Tutorial)** — any new user-facing list query on `residents`, `stylists`, `services`, `bookings`, or `log_entries` MUST add `eq(table.isDemo, false)` with comment `// is_demo filter — Phase 13`. Do NOT add the filter to internal queries (auth lookups, by-ID mutations, role checks) — only at the ~14 user-facing list-rendering sites. `logEntries` and `stylistCheckins` have no `active` column — demo records are hard-deleted by the reset endpoint and the weekly cleanup cron.
