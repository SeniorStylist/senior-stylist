...
- Remove Access: DELETE /api/facility/users/[userId] — admin-only. Guards: cannot remove yourself, cannot remove last admin. After deleting facilityUser, calls `supabase.auth.admin.signOut(userId, 'global')` via service-role admin client to invalidate the user's sessions immediately. Settings Team tab has two-step inline confirm (click "Remove" → "Remove? Yes No", mouse leave cancels). Local `localUsers` state removes the row optimistically; shows emerald toast "Access removed" for 3s.
- Invite revocation (DELETE /api/invites/[id]) also cleans up pending access_requests for the same email+facility.
