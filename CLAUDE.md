...
- PDF text extraction in `/api/services/parse-pdf`: use `pdfjs-dist/legacy/build/pdf.mjs` `getTextContent()` with position-based sorting — NOT `unpdf`. `unpdf` silently drops text layers on some PDFs (confirmed: Symphony Manor). Extract pattern: `const pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs')` — do NOT set `GlobalWorkerOptions.workerSrc`. pdfjs-dist v5 auto-detects Node.js at module load, sets `workerSrc = "./pdf.worker.mjs"` relative to `pdf.mjs`, and loads it via `await import()` in the main thread (no `worker_threads.Worker`). Overriding `workerSrc` with an absolute path breaks Vercel (path doesn't match bundle layout). Sort items by `transform[5]` (Y desc) then `transform[4]` (X asc) with 5px Y threshold, join all pages with space. `outputFileTracingIncludes` in `next.config.ts` bundles `pdf.worker.mjs` alongside `pdf.mjs` — required because pdfjs references it as a runtime string that tracing can't detect statically.
- PDF text from pdfjs-dist comes out as ONE blob with no newlines — never split('\n')
...
- PDF parser pre-extracts addon/tiered/multi_option patterns from `workingBlob` BEFORE the alternating-chunks split using global regexes (ADDON_GLOBAL, TIERED_GLOBAL, MULTI_OPTION_GLOBAL). Each match is replaced with a letter-only placeholder (`SVCPH[A-Z]+`) + dummy price `1` — no digits in the placeholder means the split regex won't fire on them. Stored services are substituted back in the main token loop when `serviceName` matches `/^SVCPH[A-Z]+$/`. The alternating-chunks split regex desyncs on ANY number embedded in a service name — pre-extraction is mandatory for entries with embedded numbers (addon amounts, tiered range numbers, ea. patterns). Extract tiered first (most specific), then multi_option, then addon (shortest/most likely to overlap). ADDON_GLOBAL character class includes `()` so service names with parenthetical descriptions (e.g. "Long Hair (longer than shoulder length) add $15 to service amount") are captured correctly.
- PDF parser garbage filter: after parsing price, skip rows matching `salon services|subject to change|copyright|services and prices`, rows starting with "or "/"and ", and rows with ≤2 words + price ≤ $5.00 (number artifacts). These guards apply AFTER price is parsed (price variable must exist before the word-count+price check).
...
- PDF parser category assignment: three fallbacks layered on top of the existing " Price " header detection. (1) Standalone category header keyword set: `CATEGORY_KEYWORDS` lists known salon headers (color, cut, perms, styling, wash, etc.); when a no-price chunk matches (or a valid-looking bare chunk), it becomes `currentCategory`. (2) Name-based heuristics (`inferCategoryFromName`): when `currentCategory` is empty or generic ("Services"), infer from service name (`color|highlight|toner` → Color, `perm|relaxer|wave` → Perms & Relaxers, etc.) — fallback ONLY, never overrides a detected header. (3) Long Hair / Matted Hair rule: these are addon services that modify the preceding service — `isAddonNameThatInherits()` matches `/^(long|matted)\s+hair\b/i` and forces category to `previousServiceCategory` (tracked at each non-addon emission), not whatever `currentCategory` is when the addon is processed.
