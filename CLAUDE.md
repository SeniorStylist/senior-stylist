...
- pdfjs-dist (v5) client-side PDF preview: `import * as pdfjsLib from 'pdfjs-dist'`; set `workerSrc` to `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsLib.version}/pdf.worker.min.js`. Add `config.resolve.alias.canvas = false` in next.config.ts webpack config — prevents pdfjs from resolving its native Node canvas dep in browser. v5 `page.render()` signature: `{ canvasContext, canvas, viewport }` (must include `canvas`, not just `canvasContext`). Store rendered data URLs (JPEG 0.85) in `previews[]`; empty string `''` = still rendering → show fallback icon.
