...
- **Sidebar amber badge for needs_review** (Phase 12C): `(protected)/layout.tsx` runs a `count(bookings WHERE needs_review = true AND active = true)` query when the user is master admin, passes `needsReviewCount` to `<Sidebar>`. The Master Admin nav link shows an amber count pill when > 0. Don't add other nav badges here — keeping the sidebar quiet is intentional.
