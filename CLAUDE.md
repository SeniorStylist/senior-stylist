...
## Layout Shell (Phase 12Y, corrected in 12Y followups)

The protected layout (`src/app/(protected)/layout.tsx`) is a 3-zone vertical flexbox shell (4-zone with desktop sidebar). The OUTER container is `h-[100dvh] flex overflow-hidden`. The INNER column is `flex-1 min-w-0 flex flex-col overflow-hidden`, containing in order: `<MobileFacilityHeader>` (shrink-0, mobile-only), `<TopBar>` (shrink-0, desktop-only), `<main className="main-content flex-1 min-h-0 overflow-y-auto overscroll-contain">`, `<MobileNav>` (shrink-0, mobile-only). **`<main>` is the ONLY vertical scroll container in the protected layout** — no nested overflow-auto.

**`html, body { height: 100% }`** in `globals.css` — guarantees the `h-[100dvh]` shell always fills the viewport (defends against ancestor collapse on iOS). Required for the bottom nav to sit flush at the viewport bottom.
...
**Where floating chrome is mounted:** viewport-anchored chrome (`<MobileDebugButton>`, `<InstallBanner>`, `<TourModeBanner>`, `<DebugBadge>`) is an **outer-shell sibling** of the inner column — OUTSIDE `<main>` — so its `fixed` positioning is viewport-relative. Page-level FABs live inside their page client (also fine — there's no containing block anywhere now). Don't mount viewport chrome inside `<main>`.
