...
- **`facilities.rev_share_percentage`** — nullable integer column. Populated via `POST /api/super-admin/import-facilities-csv` (master admin only, fuzzy-match by name, always-overwrite). `contactEmail` is fill-if-null only.
