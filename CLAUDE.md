...
- PDF text extraction in `/api/services/parse-pdf`: use `pdfjs-dist/legacy/build/pdf.mjs` `getTextContent()` with position-based sorting — NOT `unpdf`. `unpdf` silently drops text layers on some PDFs (confirmed: Symphony Manor). Extract pattern: dynamic import, `createRequire(process.cwd() + '/package.json').resolve('pdfjs-dist/legacy/build/pdf.worker.mjs')` for workerSrc, sort items by `transform[5]` (Y desc) then `transform[4]` (X asc) with 5px Y threshold, join all pages with space. Also add `outputFileTracingIncludes` in `next.config.ts` so Vercel bundles the worker file (it can't be auto-detected via dynamic require.resolve).
