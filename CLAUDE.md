...
- access_requests table: id, facility_id, email, full_name, status ('pending'|'approved'|'denied'), role, user_id, timestamps. POST /api/access-requests is public (no auth). PUT /api/access-requests/[id] approves (provisions facilityUsers row if userId known) or denies. Settings "Requests" tab shows pending list with badge count. Dashboard shows amber banner when count > 0.
