...
- OCR log import: POST /api/log/ocr accepts multipart/form-data `images[]` (plural, multiple files allowed). Calls Gemini REST API **directly via `fetch`** — do NOT use the `@google/generative-ai` SDK, it hardcodes `v1beta` in its URL and both `gemini-1.5-flash` and `gemini-1.5-flash-8b` are no longer available on v1beta. Direct call: `POST https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=...`. Supports JPEG, PNG, WEBP, GIF, and **PDF** — Gemini handles PDFs natively via `inline_data` with `mime_type`. Processes files sequentially with a 45s per-file `Promise.race` timeout guard. Extracts `{ date, stylistName, entries[] }` per sheet. Returns `{ data: { sheets: [{ imageIndex, date, stylistName, entries }] } }`. Per-image errors are included inline. GEMINI_API_KEY checked server-side (500 if missing). Route exports `maxDuration = 60` and `dynamic = 'force-dynamic'`.
