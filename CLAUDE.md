...
- OCR log import: POST /api/log/ocr accepts multipart/form-data `images[]` (plural, multiple files allowed). Calls Google Gemini 2.0 Flash (`GEMINI_API_KEY`, server-side only). Processes images sequentially. Extracts `{ date, stylistName, entries[] }` per sheet. Returns `{ data: { sheets: [{ imageIndex, date, stylistName, entries }] } }`. Per-image errors are included inline rather than aborting the whole request.
...
- OcrImportModal: `src/app/(protected)/log/ocr-import-modal.tsx`. 3-step flow: (1) Upload: multi-file picker, thumbnails; (2) Review: tab per sheet, date/stylist selectors, per-entry resident + service dropdowns with "Create new: [name]" option, amber duplicate banners when 2+ fuzzy matches, price/notes editing, Unclear badge; (3) Confirm: summary counts, import. Sheet-level stylist is required — sheets with no stylist are skipped.
