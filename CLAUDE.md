...
- Remove Access: DELETE /api/facility/users/[userId] — admin-only. Guards: cannot remove yourself, cannot remove last admin. Settings Team tab has two-step inline confirm (click "Remove" → "Remove? Yes No", mouse leave cancels). Local `localUsers` state removes the row optimistically; shows emerald toast "Access removed" for 3s. Data cleanup: use `scripts/cleanup-test-accounts.mjs` pattern (delete after running) — `sql.unsafe()` for IN queries with UUID arrays via the postgres library.
