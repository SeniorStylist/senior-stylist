...
export async function autoCollectOnCompletion(bookingId: string): Promise<void> {
  try {
    ...
    await handleFailover(result, b.residentId, amount, b.id)
  } catch (err) {
...
export async function collectResidentBalance(
  ...
  const resident = await db.query.residents.findFirst({
    ...
    columns: { id: true, autopayEnabled: true, qbOutstandingBalanceCents: true, isDemo: true },
  })
  ...
  await handleFailover(result, residentId, balance, null)
