...
// P31 — the step types + DOM helpers (isMobile/resolveQuery/firstVisibleMatch/
// waitForElement/SESSION_KEY/SESSION_TTL_MS/TourStep/TourDefinition) moved to
// tour-dom.ts so always-mounted layout components can import them WITHOUT
// pulling this whole catalog into the shared bundle. Re-exported here so every
// existing `from '@/lib/help/tours'` import keeps working.
export * from './tour-dom'
import {
  SESSION_KEY,
  SESSION_TTL_MS,
  isMobile,
...
export const TUTORIAL_CATALOG: Tutorial[] = [
  ...
  { id: 'meet-assistant', category: 'Getting Started', title: 'Meet Your Assistant', blurb: 'Ask anything in plain English, get things done with a confirm tap, and be walked through the app with on-screen arrows.', estMinutes: 1, icon: 'Sparkles', roles: ['admin', 'super_admin', 'facility_staff', 'bookkeeper', 'stylist'], tourId: 'meet-assistant', platform: 'both' },
  ...
  { id: 'master-franchise', category: 'Franchise', title: 'Franchise Management', blurb: 'Manage franchises and super admin assignments.', estMinutes: 4, icon: 'Network', roles: ['admin', 'super_admin'], tourId: 'master-franchise', masterOnly: true, platform: 'desktop' },
  { id: 'master-cross-facility-analytics', category: 'Reports', title: 'Cross-Facility Analytics', blurb: 'View revenue and KPIs across facilities.', estMinutes: 3, icon: 'BarChart3', roles: ['admin', 'super_admin'], tourId: 'master-cross-facility-analytics', masterOnly: true, platform: 'desktop' },
  { id: 'master-merge-duplicates', category: 'Residents', title: 'Merging Duplicates', blurb: 'Merge duplicate facilities and residents.', estMinutes: 4, icon: 'GitMerge', roles: ['admin', 'super_admin'], tourId: 'master-merge-duplicates', masterOnly: true, platform: 'desktop' },
  { id: 'master-team-roster', category: 'Team', title: 'Global Team Roster', blurb: 'See every user across every facility.', estMinutes: 3, icon: 'Users', roles: ['admin', 'super_admin'], tourId: 'master-team-roster', masterOnly: true, platform: 'desktop' },

  // MASTER ADMIN — MOBILE (Phase 13-Tutorial-Mobile)
  // On a phone, master admin uses the admin bottom nav. These cover the six
  ...
  { id: 'master-settings-mobile', category: 'Settings', title: 'Settings', blurb: 'Adjust facility settings; master setup lives on desktop.', estMinutes: 2, icon: 'Building2', roles: ['admin', 'super_admin'], tourId: 'master-settings-mobile', masterOnly: true, platform: 'mobile' },
]
...
export const TOUR_DEFINITIONS: Record<string, TourDefinition> = {
  ...
  'bookkeeper-scan-logs': {
    ...
    steps: [
      ...
      { route: '/log', element: '', isAction: false, title: 'Review the results', description: 'Every extracted entry appears in a table. Review each row — handwriting is not always read perfectly.' },
