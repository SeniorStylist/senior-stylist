...
// ---------------------------------------------------------------------------
// Daily Summary Email (Phase 13E — daily digest cron)
// ---------------------------------------------------------------------------

export interface DailySummaryFacility {
  facilityName: string
  facilityCode: string | null
...
export interface DailySummaryStylistRow {
  stylistName: string
  completedCount: number
  revenueCents: number
  tipCents: number
...
export function buildDailySummaryEmailHtml(params: {
  ...
  revenueCents: number
  tipCents: number
  stylists: DailySummaryStylistRow[]
}): { subject: string; html: string } {
  const { facilityName, facilityCode, dateLabel, completedCount, revenueCents, tipCents, stylists } = params
  const dollars = (c: number) => `$${(c / 100).toFixed(2)}`
  const subject = `Daily Summary — ${facilityName} · ${dateLabel}`

  const stylistRows = stylists
    .map(
      (s) => `<tr>
        <td style="padding:10px 0;border-bottom:1px solid #F5F5F4;color:#1C1917;font-size:14px;">${escHtml(s.stylistName)}</td>
        <td style="padding:10px 0;border-bottom:1px solid #F5F5F4;color:#1C1917;font-size:14px;text-align:center;">${s.completedCount}</td>
        <td style="padding:10px 0;border-bottom:1px solid #F5F5F4;color:#1C1917;font-size:14px;text-align:right;">${dollars(s.revenueCents)}</td>
        <td style="padding:10px 0;border-bottom:1px solid #F5F5F4;color:#78716C;font-size:14px;text-align:right;">${s.tipCents > 0 ? dollars(s.tipCents) : '—'}</td>
      </tr>`,
    )
    .join('')
  ...
  return {
    ...
    html: [
      ...
      '<body style="margin:0;padding:0;background:#F5F5F4;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',sans-serif;">',
      '<div style="max-width:560px;margin:40px auto;background:#fff;border-radius:16px;border:1px solid #E7E5E4;overflow:hidden;">',
      emailHeader({ eyebrow: 'Daily Summary', title: facilityName, subtitle: dateLabel, codeChip: facilityCode ?? undefined }),
      '<div style="padding:28px 32px;">',
      '<table style="width:100%;border-collapse:separate;border-spacing:8px;margin:-8px 0 20px;"><tr>',
      `<td style="background:#F9EFF2;border-radius:12px;padding:16px 20px;text-align:center;width:33%;"><p style="margin:0;color:#8B2E4A;font-size:26px;font-weight:700;">${completedCount}</p><p style="margin:4px 0 0;color:#78716C;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">Visits</p></td>`,
      `<td style="background:#F9EFF2;border-radius:12px;padding:16px 20px;text-align:center;width:33%;"><p style="margin:0;color:#8B2E4A;font-size:26px;font-weight:700;">${dollars(revenueCents)}</p><p style="margin:4px 0 0;color:#78716C;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">Revenue</p></td>`,
      `<td style="background:#F9EFF2;border-radius:12px;padding:16px 20px;text-align:center;width:33%;"><p style="margin:0;color:#8B2E4A;font-size:26px;font-weight:700;">${dollars(tipCents)}</p><p style="margin:4px 0 0;color:#78716C;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">Tips</p></td>`,
      '</tr></table>',
      stylists.length > 1 ? [
        '<h2 style="margin:0 0 12px;color:#1C1917;font-size:14px;font-weight:700;">By Stylist</h2>',
        '<table style="width:100%;border-collapse:collapse;"><thead><tr>',
        '<th style="padding:6px 0;text-align:left;color:#78716C;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid #E7E5E4;">Stylist</th>',
        '<th style="padding:6px 0;text-align:center;color:#78716C;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid #E7E5E4;">Visits</th>',
        '<th style="padding:6px 0;text-align:right;color:#78716C;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid #E7E5E4;">Revenue</th>',
        '<th style="padding:6px 0;text-align:right;color:#78716C;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;border-bottom:2px solid #E7E5E4;">Tips</th>',
        `</tr></thead><tbody>${stylistRows}</tbody></table>`,
      ].join('') : '',
      `</div>${EMAIL_FOOTER}</div></body></html>`,
    ].join(''),
  }
}
