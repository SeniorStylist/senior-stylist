...
Front desk / admin: open the Sign-Up Sheet from the dashboard (clicking the calendar or the + button opens it too), enter the resident (or create one inline), the service — "＋ Add another service" lets one visit carry several — an optional preferred date/time, and notes. There's no stylist picker: the preferred DAY drives auto-assignment to whoever is scheduled that day. The date field shows which days a stylist actually works ("Stylist days: Tuesday and Thursday") and blocks off-days — a request for a day nobody works would just sit forever. A "Pending requests" section at the top shows everything facility-wide; the × cancels a request.

Stylists: pending requests appear in an amber panel above your calendar, ordered by the day (and time) they're wanted, with a badge on the Calendar tab counting them. A green card icon next to a name means they have a card on file. Tap "Pick time →" to open the booking form pre-filled, choose a slot, and the request becomes a real appointment. On desktop you can also drag a request card straight onto the calendar.
...
- Payment-covered markers: a tiny green card icon by a resident's name means they have a card on file, and a blue coin icon means they have salon credit — you'll see them when picking residents for bookings, on daily-log rows, and in the quick-profile popup. No icon = sort out payment at the chair.
- Autopay: per facility (Settings → Billing) and per resident. It can charge automatically when a visit completes, or sweep balances nightly. A card saved during the family signup wizard turns per-visit autopay on automatically; families can also flip it on or off from their Salon Account's Billing page (a saved card is required). Everyone on the family's email list is notified when autopay changes, every automatic charge emails a receipt, and there's a safety cap on automatic amounts. Failed charges alert the office and fall back to a pay link. Whenever a card is saved — by the family or by staff — the family gets a "card saved" security email.
- Charge on finalize: when the facility's autopay mode is "on completion", tapping "Finalize day" on the Day Log also charges the saved cards of autopay residents for that day's unpaid completed visits — one charge per resident, marked paid immediately, so an edited and re-finalized day never double-charges (already-paid rows are skipped).
- Scanned sheets: after importing a scanned paper log sheet, a "Charge cards" screen lists the autopay residents from that sheet with saved cards. Nothing is charged until you review the names and press the button — scanners can mis-read similar names, so this step is always human-confirmed.
- Gift link: every resident has a shareable gift page ("Copy gift link" on their profile; families find "Share a gift link" on their Billing page). Any relative can send salon credit through it without seeing the account — they only see the facility name and the resident's first name.
