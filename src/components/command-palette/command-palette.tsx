...
export function CommandPalette({ role, isMaster, canSearchEntities, canManage = false }: CommandPaletteProps) {
  ...
  const askAiItem = useMemo<FlatItem>(
    () => ({
      ...
      id: 'ask-ai',
      route: '',
      label: query.trim().length >= 2 ? `Ask me: “${query.trim()}”` : 'Ask me anything',
      secondary: query.trim().length >= 2 ? 'Sends your question to your assistant' : 'Questions, bookings, walkthroughs — anything',
      icon: Sparkles,
    }),
    [query],
  )
