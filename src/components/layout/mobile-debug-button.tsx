...
export function MobileDebugButton({ isMaster, allFacilities }: MobileDebugButtonProps) {
  ...
  const floatingBottom = 'calc(env(safe-area-inset-bottom) + 88px)'

  return (
    <>
      {/* Floating indicator / trigger — mobile only */}
      {debugInfo ? (
        <div
          className="md:hidden fixed right-4 z-50 flex items-center gap-1.5 bg-amber-400 text-amber-950 text-xs font-bold px-3 py-2 rounded-2xl shadow-xl"
        ...
        <button
          onClick={() => setOpen(true)}
          className="md:hidden fixed right-4 z-50 bg-[#8B2E4A] text-white text-[11px] font-bold px-3 py-1.5 rounded-full shadow-lg"
          style={{ bottom: floatingBottom }}
        >
          Debug
        </button>
