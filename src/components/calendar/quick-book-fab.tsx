...
function QuickBookFAB({
  ...
  const handleSwipeEnd = (e: React.TouchEvent) => {
    ...
    // Swipe right (deltaX > 60) and more horizontal than vertical = go back
    if (deltaX > 60 && deltaX > deltaY && step > 1) {
      setStep(step - 1)
    }
  }
  ...
  // Footer rendered outside the scroll area (always visible)
  const stepFooter =
    step === 3 ? (
      <div className="px-5 pt-3 pb-4 border-t border-stone-100 space-y-1.5">
        {error && (
  ...
  return (
    <>
      ...
      >
        {/* Step indicator dots */}
        <div className="flex items-center justify-center gap-2 px-5 py-3 border-b border-stone-100">
          {[1, 2, 3].map((s) => (
            <div
              key={s}
              style={{
                ...
                width: s === step ? 16 : 6,
                borderRadius: 3,
                background: s <= step ? '#0D7377' : '#e7e5e4',
        ...
        {/* Swipe wrapper — captures horizontal swipe to go back */}
        <div
                ...
                {filteredResidents.length === 0 ? (
                  ...
                  filteredResidents.map((r) => (
                    ...
                    >
                      ...
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-stone-300 shrink-0">
