...
export function DebugBadge() {
  const [debug, setDebug] = useState<{ role: string; facilityName: string; demo?: string } | null>(null)
  ...
  return (
    <div className="hidden md:flex fixed top-4 right-4 z-[200] items-center gap-2 bg-amber-400 text-amber-950 text-xs font-bold px-3 py-2 rounded-2xl shadow-xl border-2 border-amber-500">
      <span className="text-amber-800 text-[10px] font-semibold uppercase tracking-wide">
        {debug.demo === 'apley' ? 'Apley demo' : 'Debug'}
      </span>
      ...
      >
        {/* P61 — name what you are leaving. "Exit to Master Admin" during the
            Apley walk read as unrelated chrome rather than the way out of it. */}
        {debug.demo === 'apley' ? '← End the Apley demo' : '← Exit to Master Admin'}
      </button>
    </div>
  )
}
