...
interface ImportResult {
  ...
  facilitiesWithBalance: number
  residentsWithBalance: number
  warnings: string[]
}

type State = 'upload' | 'loading' | 'results'

export function ImportBillingHistoryClient() {
  ...
  const canImport = invoicesFile !== null || transactionsFile !== null
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res = await fetch('/api/super-admin/import-billing-history', {
        method: 'POST',
        body: formData,
      })
  ...
  function reset() {
    ...
    if (transactionsRef.current) transactionsRef.current.value = ''
  }
      ...
      {state === 'loading' && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm mx-4">
            ...
            <h3 className="text-base font-semibold text-stone-900 text-center mb-1">Importing Billing History...</h3>
            <p className="text-sm text-stone-500 text-center mb-5">Processing invoices and payments — this may take a minute</p>
        ...
        {state !== 'results' ? (
          <div className="bg-white rounded-2xl border border-stone-200 p-8">
            ...
            <p className="text-sm text-stone-500 mb-8">
              Upload your QuickBooks Invoice List and/or Transaction List CSVs to import historical billing data.
              Safe to re-run — invoices upsert by number, payments skip on conflict.
            </p>
            ...
            />
            <div className="mb-4">
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-2">Invoice List CSV <span className="font-normal text-stone-400">(optional)</span></p>
              <div
                onClick={() => invoicesRef.current?.click()}
              ...
              >
                ...
                {invoicesFile ? (
                  <p className="text-sm font-medium text-stone-700">{invoicesFile.name}</p>
                ) : (
                  <>
                    <p className="text-sm font-medium text-stone-600">Click to select invoice CSV</p>
                    <p className="text-xs text-stone-400 mt-0.5">Columns: Date, Transaction type, Num, Name, Amount, Open balance</p>
                  </>
                )}
              </div>
            </div>
            ...
            />
            <div className="mb-8">
              <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-2">Transaction List CSV <span className="font-normal text-stone-400">(optional)</span></p>
              <div
              ...
              >
                ...
                {transactionsFile ? (
                  <p className="text-sm font-medium text-stone-700">{transactionsFile.name}</p>
                ) : (
                  <>
                    <p className="text-sm font-medium text-stone-600">Click to select transaction CSV</p>
                    <p className="text-xs text-stone-400 mt-0.5">QB Transaction List export with Payment rows</p>
                  </>
                )}
              </div>
            </div>
            ...
            <button
              ...
              onMouseEnter={(e) => { if (canImport) e.currentTarget.style.backgroundColor = '#72253C' }}
            ...
            <div className="bg-white rounded-2xl border border-stone-200 p-8">
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between px-4 py-3 bg-stone-50 rounded-xl">
                  <span className="text-sm font-medium text-stone-700">Invoices processed</span>
                  <span className="text-sm font-semibold text-emerald-700">{result.invoices.total.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-3 bg-stone-50 rounded-xl">
                  <span className="text-sm font-medium text-stone-700">Payments processed</span>
                  <span className="text-sm font-semibold text-emerald-700">{result.payments.total.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-3 bg-stone-50 rounded-xl">
                  <span className="text-sm font-medium text-stone-700">Facilities with open balance</span>
                  <span className="text-sm font-semibold text-stone-700">{result.facilitiesWithBalance.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-3 bg-stone-50 rounded-xl">
                  <span className="text-sm font-medium text-stone-700">Residents with open balance</span>
                  <span className="text-sm font-semibold text-stone-700">{result.residentsWithBalance.toLocaleString()}</span>
                </div>
              </div>
