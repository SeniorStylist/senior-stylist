...
export function IPView({
  ...
  async function handleSendAll() {
    if (!confirm(`Send billing reminders to ${eligibleCount} resident${eligibleCount === 1 ? '' : 's'} with outstanding balances?`)) return
    ...
    try {
      ...
      alert(`Sent ${sent ?? 0} reminder${(sent ?? 0) === 1 ? '' : 's'}${skipped ? `, ${skipped} skipped` : ''}.`)
      onRefresh()
    } catch {
      ...
      <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-stone-100">
          <h3 className="text-sm font-semibold text-stone-700">
            Residents{eligibleCount > 0 ? ` · ${eligibleCount} with balance` : ''}
          </h3>
          <div className="flex items-center gap-2">
            {eligibleCount > 0 ? (
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold bg-[#8B2E4A] text-white hover:bg-[#72253C] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {sendAllLoading ? 'Sending…' : `Send All (${eligibleCount})`}
              </button>
            ) : null}
            <DisabledActionButton label="Send via QB" title="Available after QB production approval" />
          </div>
        </div>
              ...
              <div className="md:col-span-2 md:text-right flex md:justify-end items-center gap-2">
                ...
                {canSend ? (
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-lg px-2.5 py-1 text-xs font-semibold bg-[#8B2E4A] text-white hover:bg-[#72253C] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
