// Phase 15 F4 — convert a pending waitlist entry into a real booking.
// Mirrors /api/signup-sheet/[id]/convert: same body contract, same validation,
// one atomic transaction (insert booking + flip entry to 'booked').
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const primaryServices = primaryServiceIds
      .map((sid) => primarySvcRows.find((s) => s.id === sid))
    ...
    // Atomic: create booking + flip entry to booked
    ...
    console.error('POST /api/waitlist/[id]/convert failed:', err)
