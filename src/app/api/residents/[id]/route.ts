...
export async function PUT(
  ...
  try {
    ...
    // P41 — master admin edits residents at ANY facility (row-scoped update)
    const master =
    ...
    const [updated] = await db
      ...
      .where(master ? eq(residents.id, id) : and(eq(residents.id, id), eq(residents.facilityId, facilityUser!.facilityId)))
      .returning()
