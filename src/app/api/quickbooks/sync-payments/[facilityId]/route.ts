...
// Unattended-capable READ from QB — gated behind QB_INVOICE_SYNC_ENABLED
// exactly like the invoice pull (same Intuit-approval concern, and it feeds
// the balances the autopay sweep charges against).
  ...
  try {
    if (process.env.QB_INVOICE_SYNC_ENABLED !== 'true') {
      return Response.json(
        { error: 'Payment sync not yet available — awaiting Intuit production approval' },
        { status: 503 },
      )
    }
    ...
    const result = await syncQBPayments(facilityId, { fullSync, createdBy: user.id })

    revalidateTag('billing', {})

    return Response.json({ data: result })
  } catch (err) {
    console.error('[quickbooks/sync-payments] error:', err)
    return Response.json(
