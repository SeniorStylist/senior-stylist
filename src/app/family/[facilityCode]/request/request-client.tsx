...
export function RequestClient({ facilityCode, lang, residentId, residentName, groups, workingDows = [] }: Props) {
  ...
  // P55 — the from..to window must include at least one day the stylist works
  const workingDayNames =
    workingDows.length > 0 ? formatWorkingDayNames(workingDows, lang === 'es' ? 'es' : 'en-US') : ''
  const rangeOffDays = dateMode === 'range' && !!from && !!to && !rangeHasWorkingDow(from, to, workingDows)
  ...
  const onSubmit = async () => {
    ...
    if (rangeOffDays) {
      setError(t('request.notWorkingDay', { days: workingDayNames }))
      return
    }
    setSubmitting(true)
    setError(null)
      ...
      <section className="bg-white rounded-2xl border border-stone-100 shadow-[var(--shadow-sm)] p-5">
        ...
        {dateMode === 'range' && (
          <>
            ...
            {/* P55 — the stylist only comes on certain days */}
            {rangeOffDays ? (
              <p role="alert" className="mt-2 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-xl px-3.5 py-2.5">
                {t('request.notWorkingDay', { days: workingDayNames })}
              </p>
            ) : workingDayNames ? (
              <p className="mt-2 text-xs text-stone-500">{t('request.workingDaysHint', { days: workingDayNames })}</p>
            ) : null}
          </>
        )}
      </section>
